                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12 col-lg-auto my-2">
                                            <label for="file" class="form-label">' . lang('Select .zip to Upload') . '</label>
                                            ' . $liveform->output_field(array('type'=>'file', 'id'=>'file', 'name'=>'file', 'size'=>'60', 'class'=>'form-control w-auto')) . '
                                        </div>
                                        <div class="col-12 col-lg my-2">
                                            <label for="name" class="form-label">' . lang('Tag') . '</label>
                                            ' . $liveform->output_field(array('type'=>'text', 'id'=>'name', 'name'=>'import_name', 'size'=>'10', 'class'=>'form-control')) . '
                                            <div class="form-text text-end">' . lang('appended to imported items and filenames for easy reference, recommended, e.g. \'mysite\'') . '</div>
                                        </div>
                                    </div>
                                </div>       
                            </div> 
...
// Otherwise the form has been submitted so process it.
    ...
    if (!$archive_items) {
        output_error(lang('The zip file cannot be extracted, because it is not a valid zip file.') . '<a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.');
    }
    ...
    foreach ($archive_items as $archive_item) {
        ...
        if ($type == 'html') {
            ...
            $content = preg_replace('/' . preg_quote('<!-- Start Pinegrap dynamic code -->') . '.*?' . preg_quote('<!-- End Pinegrap dynamic code -->') . '/s', '', $content);
        ...
        // Otherwise this is a regular file, so create file.
            ...
            if ($import_name != '') {
                ...
                if ($old_file) {
                    ...
                    log_activity( lang(array('string'=>'file was renamed during import my site ({var:1} -> {var:2})','vars'=>array($name,$backup_name))) );
