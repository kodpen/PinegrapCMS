...
function get_form_list_view($properties) {
    ...
    $editable = isset($properties['editable']) ? $properties['editable'] : false;
    $email    = $properties['email'];

    // System widget support: when called from _expand_system_widgets(), these are set.
    // $widget_id     — shared_components.id, used as liveform session namespace (sw{id}_*)
    // $widget_templates — compiled templates from system_region_config.templates, override DB values
    $widget_id        = isset($properties['widget_id'])   ? (int)$properties['widget_id']   : 0;
    $widget_templates = isset($properties['templates'])   ? $properties['templates']         : array();
    // param_prefix: 'sw{widget_id}_' for widgets, or '{page_id}_' for standard pages.
    // Used as the liveform session key prefix so multiple widgets on the same page
    // never share search/browse/page state.
    $param_prefix = ($widget_id > 0) ? ('sw' . $widget_id . '_') : ($page_id . '_');
    ...
    // System widget template overrides: compiled templates from Visual Pinegrap Editor take priority
    // over DB-stored templates. Only non-empty strings override (allows partial overrides).
    if (!empty($widget_templates['item']))       $layout = $widget_templates['item'];
    if (!empty($widget_templates['search']))     $header = $widget_templates['search'] . $header;
    if (!empty($widget_templates['browse']))     $header = $widget_templates['browse'] . $header;
    if (!empty($widget_templates['pagination'])) $footer = $footer . $widget_templates['pagination'];

    // URL parameter keys — use param_prefix so multiple widgets on the same page stay independent.
    // Standard pages:  '{page_id}_query', '{page_id}_browse_field_id', etc.
    // System widgets:  'sw{widget_id}_query', 'sw{widget_id}_browse_field_id', etc.
    $pk_query         = $param_prefix . 'query';
    $pk_browse_field  = $param_prefix . 'browse_field_id';
    $pk_browse_filter = $param_prefix . 'browse_filter';
    $pk_page_number   = $param_prefix . 'page_number';
    ...
    if (($browse == 1) || ($search == 1)) {
        ...
        if (
            ...
            $new_query = $liveform->get_field_value($pk_query);
        ...
        $liveform->set($pk_query, $search_query);
        ...
        if ($browse) {
            ...
            if (
                ...
                && ($liveform->field_in_session($pk_browse_field) == false)
                && ($search_query == '')
                &&
                (
                    ($search == 0)
                ...
                $liveform->assign_field_value($pk_browse_field, $browse_show_by_default_form_field_id);
            }
        ...
        if ($search_advanced) {
            ...
            foreach ($variables as $variable) {
                ...
                // This prevents warnings in PHP 7.4+ and fatal errors in PHP 8.x
                $field['name'] = isset($field['name']) && is_string($field['name']) ? mb_strtolower($field['name']) : '';
        ...
        // so prepare feature.
        if (isset($browse_enabled) && $browse_enabled) {

            $browse_field_id_options = array();
            $browse_field_id_options[lang('Browse')] = '';
            ...
            foreach ($browse_fields as $key => $browse_field) {
                ...
                if ($browse_field['id'] == $liveform->get_field_value($pk_browse_field)) {
                ...
                foreach ($browse_field['filters'] as $filter_key => $filter) {
                    ...
                    // then remember that.
                    if (
                        ($browse_field['id'] == $liveform->get_field_value($pk_browse_field))
                        && ($filter['name'] == $liveform->get_field_value($pk_browse_filter))
                    ) {
                        $filter['current'] = true;
                    }
            ...
            if (
                ($liveform->get_field_value($pk_browse_field) != '')
                && ($selected_browse_field_valid == false)
            ) {
                $liveform->assign_field_value($pk_browse_field, '');
            }

            // If there is a selected browse field, then complete various tasks.
            if ($liveform->get_field_value($pk_browse_field) != '') {
                ...
                if ($search == 1) {
                    ...
                    $liveform->assign_field_value($pk_query, '');
            ...
            $liveform->set($pk_browse_field, 'options', $browse_field_id_options);
                    ...
                    switch ($field['type']) {
                            ...
                            switch ($field['type']) {
                                ...
                                case 'date':
                                    ...
                                    $output_date_time_picker =
                                        ...
                                        <script>
                                            software_$("#' . $field['html_field_name'] . '").datepicker(datetimepicker_options);
                                        </script>';
                                ...
                                case 'date and time':
                                    ...
                                    $output_date_time_picker =
                                        ...
                                        <script>
                                            software_$("#' . $field['html_field_name'] . '").datetimepicker(datetimepicker_options);
                                        </script>';
                        ...
                        case 'radio button':
                            ...
                            $output_field .= $liveform->output_field(array(
                                ...
                                'checked' => 'checked',
                                'class' => $class)) .
                                '<label for="' . $field['html_field_name'] . '_1"> [' . lang('Any') . ']</label><br />';
    ...
    if (
        (
            ($search == 0)
            && (isset($browse_enabled) && $browse_enabled == false)
        )
        || ($show_results_by_default == 1)
        ||
        (
            ($liveform->get_field_value($pk_browse_field) != '')
            && ($liveform->get_field_value($pk_browse_filter) != '')
        )
        || ($search_query != '')
        ...
        if (
            (isset($browse_enabled) && $browse_enabled == true)
            && ($liveform->get_field_value($pk_browse_field) != '')
            && ($liveform->get_field_value($pk_browse_filter) != '')
        ) {
            $filters[] = array(
        ...
        if (isset($_GET[$pk_page_number]) == TRUE) {
            $page_number = $_REQUEST[$pk_page_number];
        ...
        foreach ($forms as $key => $form) {
            ...
            // get all Comparison Operators so they can be replaced with data
            preg_match_all('/\{if\s*(.*?\^\^(.*?)\^\^.*?)?(\s*(.*?)\s*)?(\'(.*?)\')?(\s*\}(.*?))?(\{\s*else\s*\}(.*?))?\{\s*endif\s*\}/si', $form['content'], $operators, PREG_SET_ORDER);

            // loop through all conditional
            foreach ($operators as $operator) {
                $whole_string = $operator[0];
                $operator_string = $operator[4];
                $positive_string = $operator[8];
                $negative_string = $operator[10];
                $field_name = mb_strtolower($operator[2]);
                $secondary_field_val = $operator[6];
                ...
                if ($field_name_valid == TRUE) {
                    ...
                    switch($operator_string){
                        case "=":
                        case "==":
                            if($form[$select_name] == $secondary_field_val){
                        ...
                        default:
                            //default is do not output.
                            $form['content'] = str_replace($whole_string, '', $form['content']);
                            break;
                    }                   
                }     
            }
            ...
            // Otherwise, this is a custom layout.
                ...
                if ($page_number > 1) {
                    $previous_page_url = build_url(array(
                        'url' => REQUEST_URL,
                        'parameters' => array($pk_page_number => $page_number - 1)));
                }
        ...
        if (
            (isset($browse_enabled) && $browse_enabled)
            and
            (
                ($liveform->get($pk_browse_field) != '')
                and ($liveform->get($pk_browse_filter) != '')
            )
        ) {
            $browse_active = true;
        }
    ...
    if(isset($browse_enabled)){
        $output_browser_enabled = $browse_enabled;
    }
    $output_search_query = '';
    if(isset($search_query)){
        $output_search_query = $search_query;
    }
    $content = render_layout(array(
        'page_id' => $page_id,
        'messages' => $liveform->get_messages(),
        'attributes' => $attributes,
        'browse' => $output_browser_enabled,
    ...
    if ($editable == true) {
        ...
        $content =
            '<div class="edit_mode" style="position: relative; outline: 1px dashed #805FA7; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_form_list_view.php?page_id=' . $page_id . '&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button forms" title="' . lang('Form List View') . ': '. h($current_page_name) .'"><i class="bi bi-pencil"></i></a>
            ' . $content . '
            </div>';
