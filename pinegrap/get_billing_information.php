...
function get_billing_information($properties) {
    ...
    if ($form->field_in_session('billing_first_name') == false) {
        ...
        // Pre-fill tax_number and tax_office from the contact if already set.
        if ($contact['tax_number'] != '') {
            $form->assign_field_value('tax_number', $contact['tax_number']);
        }

        if ($contact['tax_office'] != '') {
            $form->assign_field_value('tax_office', $contact['tax_office']);
        }
        ...
        $output =
            ...
            <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/billing_information.php" method="post">
                ...
                <table class="custom_fields" style="margin-bottom: 1.5em">
                    ...
                    <tr>
                        <td>VKN / TCKN</td>
                        <td>' . $form->output_field(array('type'=>'text', 'name'=>'tax_number', 'maxlength'=>'11', 'class'=>'software_input_text')) . '</td>
                    </tr>
                    <tr>
                        <td>' . lang('Tax Office') . '</td>
                        <td>' . $form->output_field(array('type'=>'text', 'name'=>'tax_office', 'maxlength'=>'100', 'class'=>'software_input_text')) . '</td>
                    </tr>
                    ' . $output_opt_in . '
                    ' . $output_po_number . '
                    ' . $output_referral_source . '
    ...
    // Otherwise the layout is custom.
        ...
        $form->set('tax_number', 'maxlength', 11);
        $form->set('tax_office', 'maxlength', 100);
