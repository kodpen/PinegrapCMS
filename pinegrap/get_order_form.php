...
function get_order_form($properties) {
    ...
    if ($layout_type == 'system') {
        ...
        switch ($product_layout) {
            ...
            case 'list':
                ...
                foreach ($products as $product) {
                    ...
                    if ($editable) {
                        $edit_button = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_product.php?id=' . $product['id'] . '&from=pages&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . lang('Product') . ': ' . h($product['short_description']) . '"><i class="bi bi-pencil"></i></a>';
                    }
                    ...
                    if (($available == TRUE) && ($product['recurring'] == 1) && ($product['recurring_schedule_editable_by_customer'] == 1)) {
                            ...
                            $output_start_date_row =
                                '<tr>
                                    ...
                                    <td>
                                        ...
                                        <script>
                                            software_$("#recurring_start_date_' . $product['id'] . '").datepicker(datetimepicker_options);
                                        </script>
                                    </td>
                                </tr>';
                        }
                        ...
                        $output_recurring_schedule_row .=
                                    '<fieldset class="software_fieldset" style="margin-bottom: 1em">
                                        <legend class="software_legend">' . lang('Payment Schedule') . '</legend>
                                            <table>
                                                <tr>
                                                    <td>' . lang('Frequency') . '*:</td>
                                                ...
                                                <tr>
                                                    <td>' . lang('Number of Payments') . $output_recurring_number_of_payments_required_asterisk . ':</td>
                ...
                if (($non_donations_exist == true) || ($donations_exist == false)) {
                    ...
                    $output_price_heading = lang('Amount');
                }
            ...
            case 'drop-down selection':
                ...
                $output_product_table =
                    '<table style="margin-bottom: 1em">
                        <tr id="product_row">
                            <td style="vertical-align: top"><strong>' . lang('Item') . ':</strong></td>
        ...
        if ($available_products_exist == TRUE) {
            ...
            $output_add_button = '<button type="submit" name="submit_add" value="' . $output_add_button_value . '" class="software_input_submit_primary add_button" >' . $output_add_button_label . '</button>';
        }
    ...
    // Otherwise the layout is custom.
        ...
        if ($product_layout == 'list') {
            ...
            foreach ($products as $key => $product) {
                ...
                // if page is being viewed in an editable mode, output edit button to allow user to edit product
                if ($editable) {
                    $product['edit'] = '<a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_product.php?id=' . $product['id'] . '&from=pages&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . lang('Product') . ': ' . h($product['short_description']) . '"><i class="bi bi-pencil"></i></a>';
                }
