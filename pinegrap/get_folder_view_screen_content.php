...
{   
    ...
    if (
        ...
        if ($start_page_name != '') {
            $output_my_start_page = '<div class="my_start_page" style="margin-bottom: 1em"><a class="software_button_tiny_secondary" href="' . OUTPUT_PATH . h(encode_url_path($start_page_name)) .'">' . lang('My Start Page') . '</a></div>';
        }
    }
    ...
    if (
        ...
        $liveform->mark_error('', lang('Sorry, neither Pages nor Files are selected to be included in this Folder View, so there is nothing to display.'));
    ...
    // Otherwise pages and/or files are selected to be included, so get folder view tree
        ...
        if ($output_folder_view_tree == '') {
            ...
            if (
                ...
                $output_items = lang(array('string'=>'page{suffix:1} or file{suffix:2}','suffix'=>array('s','s') ));
              

            // Otherwise if pages are included, then prepare message for that.
            } else if ($include_pages == 1) {
                $output_items = lang(array('string'=>'page{suffix:1}','suffix'=>'s'));

            // Otherwise files are only included, so prepare message for that.
            } else {
                $output_items = lang(array('string'=>'file{suffix:1}','suffix'=>'s'));
            }

            $output_folder_view_tree = '<p class="no_items_message">' . lang(array('string'=>'Sorry, we could not find any {var:1} that you have access to, for this view.','vars'=>$output_items)) . '
