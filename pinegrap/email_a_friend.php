...
$liveform->validate_required_field('from_email_address', lang('Your E-mail Address is required.'));
$liveform->validate_required_field('recipients_email_address', lang('Recipient\'s E-mail Address is required.'));
$liveform->validate_required_field('subject', lang('Subject is required.'));
...
if ($liveform->check_field_error('from_email_address') == false) {
    if (validate_email_address($liveform->get_field_value('from_email_address')) == false) {
        $liveform->mark_error('from_email_address', lang('From E-mail Address is invalid.'));
    }
}
...
if ($liveform->check_field_error('recipients_email_address') == false) {
    if (validate_email_address($liveform->get_field_value('recipients_email_address')) == false) {
        $liveform->mark_error('recipients_email_address', lang('Recipient\'s E-mail Address is invalid.'));
    }
}
...
if ($liveform->check_form_errors() == false) {
    ...
    if ($edit_rights == false) {
        ...
        if ($row[0] >= 10) {
            $liveform->mark_error('', lang('You have sent the maximum number of e-mails that you are allowed to send in one day. Please try again later.'));
        }
    }
}
