...
if (mysqli_num_rows($result) == 0) {
    output_error(lang('The page, for the form you submitted, can no longer be found.') . ' <a href="javascript:history.go(-1);">' . lang('Go back') . '</a>.');
}
...
if ($auto_registration == 0) {
    ...
    if (!USER_LOGGED_IN && $private && $private_folder_id) {
        log_activity(lang(array('string'=>'access denied to submit data to custom form ({var:1}) that grants private access because user is not logged in','vars'=>$form_name)), $_SESSION['sessionusername']);
        output_error(lang('You do not have access to submit data to this form because you are not logged in.') . ' <a href="javascript:history.go(-1);">' . lang('Go back') . '</a>.');
    }
}
...
if (($membership == 1) && ($membership_days > 0) && ($member_id != '')) {
    output_error(lang('You may not submit a membership trial form, because you are a member or have been a member.'));
}
...
if (check_view_access($folder_id) == false) {
    log_activity(lang(array('string'=>'access denied to submit data to custom form ({var:1})','vars'=>$form_name)), $_SESSION['sessionusername']);
    output_error(lang('You do not have access to submit data to this form.') . ' <a href="javascript:history.go(-1);">' . lang('Go back') . '</a>.');
}
...
if (!$enabled) {
    output_error(lang('The form is disabled.'));
}
...
if ($liveform->check_form_errors() == false) {
    ...
    foreach ($fields as $field) {
        ...
        // else this field does not have multiple values
            ...
            if (($field['contact_field']) && ($connect_to_contact != 'false')) {
                // if contact_field is image and type is file upload we can use file id for connect to contact image
                if(($field['contact_field'] == 'image') && ($field['type'] == 'file upload')){
                    $sql_update_contact .= "file_id = '" . escape($file_id) . "', ";
                    // we remove image data. we dont need it while field type changed
                    $sql_update_contact .= "image = '', ";
                }else{
                    // if this is not file upload but contact_field is image than remove file_id.
                    if($field['contact_field'] == 'image'){
                        $sql_update_contact .= "file_id = '0', ";
                    }
    ...
    if ($offer && $offer_id) {
        ...
        if ($offer_code != '') {
            ...
            if ($eligible == true) {
                ...
                log_activity(lang(array('string'=>'key code ({var:1}) was created because a custom form, that grants an offer, was submitted','vars'=>$reference_code)), $_SESSION['sessionusername']);
            }
        }
    }
                ...
                if ($submitter_email_format == 'plain_text') {
                    ...
                    if (
                        ...
                        $body .=
                            ...
                            lang('New Account') . ':' . "\n" .
                            '------------' . "\n" .
                            lang('We have created a new account for you on our site. You can find your login info below.') . "\n" .
                            "\n" .
                            lang('Email') . ': ' . $_SESSION['software']['custom_form_auto_registration'][$form_id]['email_address'] . "\n" .
                            lang('Password') . ': ' . $_SESSION['software']['custom_form_auto_registration'][$form_id]['password'];
                    }
        ...
        if ($administrator_email_format == 'plain_text') {
            ...
            if ($_SESSION['software']['custom_form_auto_registration'][$form_id]['email_address'] != '') {
                $body .=
                    ...
                    lang('New Account') . ':' . "\n" .
                    '------------' . "\n" .
                    lang('A new account was created for this user. You can find the login info below.') . "\n" .
                    "\n" .
                    lang('Email') . ': ' . $_SESSION['software']['custom_form_auto_registration'][$form_id]['email_address'] . "\n" .
                    lang('Password') . ': ' . $_SESSION['software']['custom_form_auto_registration'][$form_id]['password'];
            }
    ...
    //create Submit form notification.
    create_notification(array('action'=>'form_submited', 'type'=>'success','form_id'=>$form_id, 'title'=>$reference_code,'user'=>$username));
