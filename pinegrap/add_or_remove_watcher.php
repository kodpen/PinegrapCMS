...
// Otherwise the user is just attempting to add/remove him/herself, so handle request in a different way.
    ...
    if ($action == 'add') {
        ...
        // if the user is already a watcher, then output error
        if (mysqli_num_rows($result) > 0) {
            output_error(lang('You may not add yourself as a watcher because you are already a watcher. <a href="javascript:history.go(-1);">Go back</a>.'));
        }
    ...
    // else the user selected to remove him/herself from being a watcher, so do that
        ...
        if (mysqli_num_rows($result) == 0) {
            output_error(lang('You may not remove yourself as a watcher because you are not currently a watcher. <a href="javascript:history.go(-1);">Go back</a>.'));
        }
