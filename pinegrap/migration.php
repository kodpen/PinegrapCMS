...
if (!$_POST) {
    ...
    pg_page_shell([
        ...
        'icon'=>'design',
        'heading'=>lang('Migration'),
        'heading_description' => lang('Copy items from a source site to this site.'),
    ]) . '
                ...
                <form action="migration.php" method="post">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card my-4">
                                <div class="card-header bg-reset border-0 text-uppercase h5 text-primary fw-bold">
                                    ' . lang('Source Site Login Info') . '
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12 col-md-8 my-2">
                                            <label for="address_name" class="form-label">' . lang('API URL') . '</label>
                                            <div class="input-group ">
                                                <label for="address_name" class="input-group-text material-icons" title="' . lang('API URL') . '" data-bs-content="' . lang('Example') . ': https://www.example.com/software_directory/api.php">public</label>
                                                ' . $liveform->output_field(array('type'=>'text', 'name'=>'url', 'value' => 'https://', 'class'=>'form-control')) . '
                                            </div>
                                            <div class="form-text text-end">' . lang('If source site supports it, make sure to use \'https\' URL so login info is sent securely.') . '</div>
                                        </div>
                                        <div class="col-12"></div>
                                        <div class="col-12 col-sm-6 col-lg-4 my-1">
                                            <label class="form-label" for="username">' . lang('Email') . '</label>
                                            ' . $liveform->output_field(array('type'=>'text', 'name'=>'username', 'id'=>'username', 'class'=>'form-control', 'size'=>'30')) . '
                                        </div>
                                        <div class="col-12 col-sm-6 col-lg-4 my-1">
                                            <label class="form-label" for="username">' . lang('Password') . '</label>
                                            ' . $liveform->output_field(array('type'=>'password', 'name'=>'password', 'id'=>'password', 'class'=>'form-control', 'size'=>'30')) . '
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-12">
                            <div class="card my-4">
                                <div class="card-header bg-reset border-0 text-uppercase h5 text-primary fw-bold">
                                    ' . lang('Items to Migrate') . '
                                </div>
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12 my-2">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type'=>'checkbox', 'id'=>'styles', 'name'=>'styles', 'value'=>'1', 'class'=>'form-check-input')) . '
                                                <label class="form-check-label" for="styles">' . lang('Page Styles') . '</label>
                                            </div>
                                        </div>
                                        <div class="col-12 my-2">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type'=>'checkbox', 'id'=>'common_regions', 'name'=>'common_regions', 'value'=>'1', 'class'=>'form-check-input')) . '
                                                <label class="form-check-label" for="common_regions">' . lang('Common Regions') . '</label>
                                            </div>
                                        </div>
                                        <div class="col-12 my-2">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type'=>'checkbox', 'id'=>'designer_regions', 'name'=>'designer_regions', 'value'=>'1', 'class'=>'form-check-input')) . '
                                                <label class="form-check-label" for="designer_regions">' . lang('Designer Regions') . '</label>
                                            </div>
                                        </div>
                                        <div class="col-12 my-2">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type'=>'checkbox', 'id'=>'folders_and_pages', 'name'=>'folders_and_pages', 'value'=>'1', 'class'=>'form-check-input')) . '
                                                <label class="form-check-label" for="folders_and_pages">' . lang('Folders & Pages') . '</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <nav class="buttons navigation text-center position-sticky mb-4" style="bottom:.5rem;" aria-label="data edit buttons ">
                        <div class="container">
                            <div class=" btn-group flex-wrap justify-content-center">
                                <button type="submit" id="submit_migrate" name="submit_migrate" value="Migrate" class="btn my-1  btn-success " data-loading-content="' . lang(array('string'=>'Migrating') ) . '"><span class="material-icons me-2">drive_file_move</span><span class="btn-text">' . lang(array('string'=>'Migrate') ) . '</span></button>
                            </div>
                        </div>
                    </nav>
                </form>
...
// Otherwise the form was just submitted, so process form.
    ...
    $liveform->validate_required_field('url', lang(array('string'=>'{var:1} is required','vars'=>lang('API URL'))) );
    $liveform->validate_required_field('username', lang(array('string'=>'{var:1} is required','vars'=>lang('Email'))));
    $liveform->validate_required_field('password', lang(array('string'=>'{var:1} is required','vars'=>lang('Password'))));
    ...
    if (
        ...
        $liveform->mark_error('', lang('Please select at least one type of item to migrate.'));
    }
    ...
    if ($liveform->check_form_errors() == false) {    
        ...
        if ($response['status'] == '') {
            $liveform->mark_error('url', lang('Sorry, there was a communication error with the source site. Please check to make sure the API URL is correct.'));
    ...
    if (
        ...
        // Otherwise the request failed, so add error.
            ...
            if ($response['message'] != '') {
                $output_error_message = ' ' . lang('Error') . ': ' . h($response['message']);
            }

            $liveform->mark_error('styles', lang('Page Styles could not be retrieved from the source site, so the migration was aborted.') . $output_error_message);
        }
    }
    ...
    if (
        ...
        // Otherwise the request failed, so add error.
            ...
            $liveform->mark_error('common_regions', lang('Common Regions could not be retrieved from the source site, so the migration was aborted.') . $output_error_message);
        }
    }
    ...
    if (
        ...
        // Otherwise the request failed, so add error.
            ...
            $liveform->mark_error('designer_regions', lang('Designer Regions could not be retrieved from the source site, so the migration was aborted.') . $output_error_message);
        }
    }
    ...
    if (
        ...
        // Otherwise the request failed, so add error.
            ...
            $liveform->mark_error('folders_and_pages', lang('Folders could not be retrieved from the source site, so the migration was aborted.') . $output_error_message);
        }
    }
    ...
    if (
        ...
        // Otherwise the request failed, so add error.
            ...
            $liveform->mark_error('folders_and_pages', lang('Pages could not be retrieved from the source site, so the migration was aborted.') . $output_error_message);
        }
    }
    ...
    if (
        ...
        $message_detail .= lang(array('string'=>'{var:1} Page Style{suffix:1}','vars'=>number_format($number_of_styles),'suffix'=>$plural_suffix));
    }
    ...
    if (
        ...
        $message_detail .= lang(array('string'=>'{var:1} Common Region{suffix:1}','vars'=>number_format($number_of_common_regions),'suffix'=>$plural_suffix));
    ...
    if (
        ...
        $message_detail .= lang(array('string'=>'{var:1} Designer Region{suffix:1}','vars'=>number_format($number_of_designer_regions),'suffix'=>$plural_suffix));
    ...
    if (
        ...
        $message_detail .= lang(array('string'=>'{var:1} Folder{suffix:1}','vars'=>number_format($number_of_folders),'suffix'=>$plural_suffix));
        ...
        $message_detail .= lang(array('string'=>'{var:1} Page{suffix:1}','vars'=>number_format($number_of_pages),'suffix'=>$plural_suffix));
    }

    log_activity(lang(array('string'=>'migration was completed ({var:1})','vars'=>$message_detail)), USER_USERNAME);

    $liveform->add_notice(lang(array('string'=>'The items that you requested have been migrated ({var:1}).','vars'=>$message_detail)));
