...
if ((USER_ROLE > 0) && (USER_ROLE >= $user['role'])) {
    log_activity(lang(array('string'=>'access denied for user to login as different user because user\'s role is not high enough ({var:1} -> {var:2})','vars'=>array( USER_USERNAME, $user['username'] ))), USER_USERNAME);
...
log_activity(lang(array('string'=>'user logged in as different user ({var:1} -> {var:2})','vars'=>array( USER_USERNAME, $user['username'] ))), USER_USERNAME);
