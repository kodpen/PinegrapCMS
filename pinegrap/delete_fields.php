...
if ((isset($_POST['page_id'])) && ($_POST['page_id'] != '')) {
    ...
    if (check_edit_access($folder_id) == false) {
        log_activity(lang(array('string'=>'access denied to delete fields from {var:1} because user does not have access to modify folder that {var:1} is in','vars'=>$form_type_name)), $_SESSION['sessionusername']);
...
if ($_POST['fields']) {
    ...
    if ($form_type == 'product') {
        ...
        if ($number_of_fields > 0) {
            ...
            $liveform->add_notice( lang(array('string'=>'{var:1} field(s) have been deleted.','vars'=>$number_of_fields)) );
    ...
    // else, this is a form for a page, so update last modified info for page and log activity
        ...
        if ($number_of_fields > 0) {
            ...
            log_activity(lang(array('string'=>'{var:1} field(s) were deleted from page ({var:2}), form ({var:3})','vars'=>array($number_of_fields,$page_name,$form_name) )), $_SESSION['sessionusername'] );
    ...
    if ($access_error_exists == true) {
        $liveform->mark_error('access_error', lang('At least one field could not be deleted, because you do not have access to the field(s).'));
    }
    
// else no fields were selected
} else {
    $liveform->mark_error('no_fields_selected_error', lang('No fields were deleted because none were selected.'));
}
