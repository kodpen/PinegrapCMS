                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-12 my-2">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type'=>'checkbox', 'name'=>'update_existing_products', 'id'=>'update_existing_products', 'value'=>'1', 'class'=>'form-check-input')) . '
                                                <label class="form-check-label" for="update_existing_products">' . lang('Update Existing Products') . '</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>       
                    ...
                    <nav class="buttons navigation text-center position-sticky mb-4" style="bottom:.5rem;" aria-label="data edit buttons ">
                        <div class="container">
                            <div class=" btn-group flex-wrap justify-content-center">
                                <button type="submit" id="submit_import_products" name="submit_import_products" value="Import Products" class="btn my-1  btn-success " data-loading-content="' . lang(array('string'=>'Importing') ) . '" onclick="if(document.getElementById(\'update_existing_products\').checked == true){event.preventDefault(); var b=this; pgConfirm({title:\'' . lang('Warning') . '\', message:\'' . lang('WARNING: If an existing product has the same name as a product in the import file, then the existing product data will be replaced.') . '\', confirmText:\'' . lang('Import Products') . '\', cancelText:\'' . lang('Cancel') . '\', variant:\'warning\'}).then(function(ok){if(ok){b.disabled=true;b.form.appendChild(Object.assign(document.createElement(\'input\'),{type:\'hidden\',name:\'submit_import_products\',value:\'Import Products\'}));b.form.submit();}}); return false;}"><span class="material-icons me-2">file_upload</span><span class="btn-text" >' . lang(array('string'=>'Import Products') ) . '</span></button>
                            </div>
                        </div>
                    </nav>
                </form>
...
// Else, the form has been submitted, so process submitted data.
    ...
    if (($product_imported_count > 0) && ($product_updated_count > 0)) {
        ...
        log_activity( lang(array('string'=>'{var:1} {var:3} have been imported, and {var:2} {var:3} have been updated.','vars'=>array( $product_imported_count, $product_updated_count,lang('product(s)')))) , $_SESSION['sessionusername']);
