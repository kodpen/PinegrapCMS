...
<dl class="dl-horizontal">
    ...
    <?php else:?>
        ...
        <dd><strong><?=$reference_code?></strong></dd>
    <?php endif ?>
	...
	<div class="col-lg-9">
		<table class="table mobile_stacked">
			...
			<?php
				...
				?>
			<tr>
				<td colspan="<?=(2+$number_of_shipped_quantity_columns)?>">
					Shipping Method:
...
<div class="buttons">
	<?php if ($status === 'incomplete'):?>  
		<?php // if this incomplete order is not the active order, then prepare to output retrieve button
            ...
            <?php endif;?>
    <?php else:?>
    	<a href="<?=OUTPUT_PATH?><?=OUTPUT_SOFTWARE_DIRECTORY?>/order_history_reorder.php?id=<?=$order_id?><?=get_token_query_string_field()?>" class="software_button_primary">Reorder</a>&nbsp;&nbsp;&nbsp;
    <?php endif;?>
    <a href="<?=h($my_account_url) ?>" class="software_button_secondary back_button"><?=lang('Back') ?></a>
    <?php if ($status === 'incomplete'):?>  
    	&nbsp;&nbsp;&nbsp;<a href="<?=OUTPUT_PATH?><?=OUTPUT_SOFTWARE_DIRECTORY?>/order_history_delete_order.php?id=<?=$order_id?><?=get_token_query_string_field()?>" class="software_button_secondary delete_button" onclick="return confirm('The order will be deleted')">Delete</a>
    <?php endif;?>
</div>