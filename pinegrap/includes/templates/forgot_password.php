...
<form <?=$attributes?>>
	...
	<div class="form-group">
		<label for="email"><?=h(lang('Email'))?></label>
		<input type="email" name="email" id="email" class="form-control">
	</div>
	<button type="submit" class="btn btn-primary"><?=h(lang('Send Email'))?></button>
	<?php elseif ($screen == 'password_hint'): ?>
	<p><?=h(lang('Your Password Hint is'))?>: <strong><?=h($password_hint)?></strong></p>
	<p><?=h(lang('Do you remember your password now?'))?></p>
	<a href="<?=h($send_to)?>" class="btn btn-primary"><?=h(lang('Yes, I Remember'))?></a>
	<button type="submit" class="btn btn-primary"><?=h(lang('No, I Forget'))?></button>
	<?php endif ?>
