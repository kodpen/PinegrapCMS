...
if (validate_date(prepare_form_data_for_output($end_date, 'date')) == false) {
    output_error(lang('The end date is invalid.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>');
}
...
if (count($products) > 0) {
    ...
    $output_report =
        '<table class="chart shipping_report table-hover table " style="width:100%;display:none">
            <thead>
                <tr>
                    <th>' . lang('Product') . '</th>
                    <th style="text-align: center"><a href="view_orders.php?reset=true&amp;status=complete_or_exported&amp;advanced_filters=true' . $output_view_orders_shipping_status . '&amp;date_type=ship_date&amp;start_month=01&amp;start_day=01&amp;start_year=2000&amp;stop_month=' . date('m', strtotime('-1 day', strtotime($start_date))) . '&amp;stop_day=' . date('d', strtotime('-1 day', strtotime($start_date))) . '&amp;stop_year=' . date('Y', strtotime('-1 day', strtotime($start_date))) . '">' . lang('Before')  . '</a></th>
                    ' . $output_date_heading_cells . '
                    <th style="text-align: center"><a href="view_orders.php?reset=true&amp;status=complete_or_exported&amp;advanced_filters=true' . $output_view_orders_shipping_status . '&amp;date_type=ship_date&amp;start_month=' . date('m', strtotime('+1 day', strtotime($end_date))) . '&amp;start_day=' . date('d', strtotime('+1 day', strtotime($end_date))) . '&amp;start_year=' . date('Y', strtotime('+1 day', strtotime($end_date))) . '&amp;stop_month=12&amp;stop_day=31&amp;stop_year=2030">' . lang('After') . '</a></th>
                    <th style="text-align: center"><a href="view_orders.php?reset=true&amp;status=complete_or_exported&amp;advanced_filters=true' . $output_view_orders_shipping_status . '&amp;date_type=ship_date&amp;start_month=01&amp;start_day=01&amp;start_year=2000&amp;stop_month=12&amp;stop_day=31&amp;stop_year=2030">' . lang('Total') . '</a></th>
                </tr>
            </thead>
            <tbody>' . $output_product_rows . '</tbody>
            <tfoot>
                <tr class="fw-bold table-info">
                    <td style="font-weight: bold">' . lang('Grand Total') . '</td>
...
// Otherwise there is not any data for any products, so output notice.
    ...
    $output_report = '<div class="alert alert-info mb-0">' . lang('Sorry, there does not appear to be any shipping info for that date range.  Shipping info will appear for this report once shippable Products are purchased.') . '</div>';
}

print
    pg_page_shell(
        array(
            'title'=> lang('Shipping Report'),
            'extra classes'=>'store',
            'icon'=>'store', 
            'heading'=> lang('Shipping Report'),
            'heading_description' => lang('View the number of Products that need to be shipped or have been shipped on different days.'),
            'cancel'=>false
        )
    ) . '
            ...
            <div class="row mb-2  flex-wrap">
                <div class="col-12 col-sm-12 col-md-6 col-xl-8 text-center text-md-start">
                    ...
                    <nav id="button_bar" class="navigation " aria-label="Button Bar">
                        <a class="btn btn-sm btn-primary m-1 " href="view_ship_date_adjustments.php" data-loading-content="' . lang(array('string'=>'Loading') ) . '"><span class="bi bi-wrench-adjustable-circle bi-me-2"></span>' . lang('Ship Date Adjustments') . '</a>
                    </nav>
                </div>
 
                <div class="col-12 col-sm-12 col-md-6 col-xl-4 align-self-end">
                    <div class="row justify-content-center justify-content-md-end">
                        <div class="row justify-content-center justify-content-md-end" >
                            <form id="update_status" method="get" class="search_form disable_shortcut col-auto">
                                 <div class="input-group input-group-sm">
                                    <label class="input-group-text mt-1 mb-1" title="' . lang('Status') . '" for="filter">' . lang('Status') . '</label>
                                    <select id="status" name="status" class="form-select mt-1 mb-1" onchange="document.getElementById(\'update_status\').submit()">
                                        <option value="any"' . $status_any_selected . '>[' . lang('Any') . ']</option>
                                        <option value="unshipped"' . $status_unshipped_selected . '>' . lang('Unshipped') . '</option>
                                        <option value="shipped"' . $status_shipped_selected . '>' . lang('Shipped') . '</option>
                                    </select>
                                </div>
                                <div class="btn-group btn-group-sm col-auto py-0 px-1 my-1">
                                    <a class="btn py-0 px-1 border-start border-top border-bottom" href="view_shipping_report.php?start_date=' . $output_previous_start_date . '&end_date=' . $output_previous_end_date . '"><</a>
                                    <span class="border  badge rounded-0 fw-light border-2">    ' . $output_date_range . '</span>
                                    <a class="btn py-0 px-1 border-end border-top border-bottom" href="view_shipping_report.php?start_date=' . $output_next_start_date . '&end_date=' . $output_next_end_date . '">></a>
                                </div>
                            </form>
                        </div>
                    </div>
