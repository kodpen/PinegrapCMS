    ...
    // Otherwise the confirmation message should not be shown, so determine what should be shown.
        ...
        // or the visitor came from the control panel, so determine if custom form should be shown.
            ...
            if ($enabled) {
                ...
                if ($layout_type == 'system') {
                        ...
                        $output_watcher_check_box =
                            '<tr class="add_watcher_row">
                                ...
                                <td style="padding-top: 1em; padding-bottom: 1em">' .
                                    ...
                                    '<label for="watcher"> ' . lang(array('string'=>'Notify me when a {var:1} is added.','vars'=>h(mb_strtolower(get_comment_label(array('label' => $watcher_page['comments_label'])))))) . '</label>
                                </td>
                            </tr>';
                    }
                    ...
                    if ($submit_button_label) {
                        $output_submit_button_value = h($submit_button_label);
                        $output_submit_button_label = h($submit_button_label);
                    
                    // else a submit button label could not be found, so use a default label
                    } else {
                        $output_submit_button_value = 'Submit';
                        $output_submit_button_label = lang('Submit');
                    }
                            ...
                            <table style="margin-bottom: 15px">
                                ...
                                <tr class="submit_button_row">
                                    <td colspan="2" style="padding-top: 1em">
                                        ' . $output_save_button . '
                                        <button type="submit" name="submit_button" value="' . $output_submit_button_value . '" class="software_input_submit_primary submit_button">' . $output_submit_button_label . '</button>&nbsp;&nbsp;
                                    </td>
                                </tr>
                            </table>
                        </form>';
                ...
                // Otherwise the layout is custom.
                    ...
                    if (isset($_GET['connect_to_contact']) && trim(mb_strtolower($_GET['connect_to_contact'])) == 'false') {
                    ...
                    foreach ($fields as $field) {
                        ...
                        if ($set_default_values) {
                            $value_from_query_string = '';
                            if(isset($_GET['value_' . $field['id']])){
                    ...
                    if ($date_fields || $date_and_time_fields) {
                        if ($date_and_time_fields) {
                            $system .= get_date_time_picker_format();
                        }else{
                        ...
                        foreach ($date_fields as $date_field) {
                            $system .=
                                '<script>
                                    software_$("#' . escape_javascript($date_field) . '").datepicker(datetimepicker_options);
                                </script>';
                        }
                        ...
                        if ($date_and_time_fields) {
                            ...
                            foreach ($date_and_time_fields as $date_and_time_field) {
                                $system .=
                                    '<script>
                                        software_$("#' . escape_javascript($date_and_time_field) . '").datetimepicker(datetimepicker_options);
                                    </script>';
                            }
                        }
                    }
                    ...
                    if(isset($captcha_info['question'])){
                        $question = $captcha_info['question'];
                    }
            ...
            if ($editable == true) {
                ...
                $output =
                    '<div class="edit_mode" style="position: relative; outline: 1px dashed #805FA7; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/view_fields.php?page_id=' . $current_page_id . '&from=pages&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button forms" title="' . $output_title . '"><i class="bi bi-pencil"></i></a>
                        ' . $output . '
                    </div>';
            }
