...
function get_view_order_screen_content($properties)
{
    ...
    if ($order_user_id != $user_id) {
        output_error(lang('You do not have access to this order.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.');
    }
            ...
            foreach ($ship_tos as $ship_to_id) {
                ...
                foreach ($order_items as $order_item) {
                    ...
                    if ($shipped_quantities_exist == TRUE) {
                        ...
                        if ($show_shipped_quantity == 1) {
                            $output_shipped_quantity_cell = '<td class="mobile_left" style="vertical-align: top; text-align: center; white-space: nowrap; margin-left: .5em">(' . pg_format_number($shipped_quantity, 0) . ')</td>';
                            
                        // else this order item does not have a shipped quantity, so output empty cell
                        } else {
                            $output_shipped_quantity_cell = '<td class="mobile_left" style="vertical-align: top; text-align: center; white-space: nowrap;">-</td>';
                        }
                    }
                    ...
                    if ((ECOMMERCE_SHIPPING == true) && ($ship_to_id != 0)) {
                        ...
                        $output_shipping =
                            '<tr class="ship_tos data">
                                ...
                                <td class="mobile_left" colspan="' . (1 + $number_of_shipped_quantity_columns) . '" style="vertical-align: top">' . $shipping_description . '</td>
                                <td class="mobile_hide" style="vertical-align: top">&nbsp;</td>
                                <td class="mobile_hide" style="vertical-align: top">&nbsp;</td>
    ...
    // Otherwise this is a custom layout.
        ...
        foreach ($recipients as $key => $recipient) {
            ...
            if ($status != 'incomplete') {
                ...
                // if there is at least one shipping tracking number, then output them
                if (count($shipping_tracking_numbers) > 0) {
                    $recipient['shipping_tracking_numbers'] = $shipping_tracking_numbers;
                }
            }
        ...
        $content = render_layout(array(
            ...
            'shipped_quantities_exist' => $shipped_quantities_exist,
            'number_of_shipped_quantity_columns' => $number_of_shipped_quantity_columns,
