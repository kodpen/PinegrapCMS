...
if ($user['role'] == 3) {
    log_activity(lang('access denied to add calendar event location'), $_SESSION['sessionusername']);
...
if (!$_POST) {
    ...
    pg_page_shell(
        array(
            'title'=> lang(array('string'=>'Create {var:1}','vars'=>lang('Event Location'))),
...
// else the form has been submitted
    ...
    log_activity(lang(array('string'=>'{var:1} ({var:2}) was created','vars'=>array(lang('event location'),$liveform->get_field_value('name') ))), $_SESSION['sessionusername']);
    
    
    $liveform_view_calendar_event_locations = new liveform('view_calendar_event_locations');
    $liveform_view_calendar_event_locations->add_notice(lang(array('string'=>'{var:1} was created successfully','vars'=>lang('Event Location') )));
