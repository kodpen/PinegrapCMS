...
function duplicate_page($request) {
    ...
    if (($user['role'] == '3') && ($user['create_pages'] == FALSE)) {
        log_activity(lang('access denied because user does not have access to duplicate pages'));
    ...
    if (!$row) {
        return error_response(lang('Sorry, the page could not be found.'));
    }
    ...
    log_activity(lang(array('string'=>'{var:1} ({var:2}) was duplicated','vars'=>array(lang('page'), $original_page_name) )), $_SESSION['sessionusername']);
