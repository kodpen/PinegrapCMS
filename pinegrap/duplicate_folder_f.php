...
function duplicate_folder($request) {
    ...
    if (!$folder) {
        return error_response(lang('Sorry, the folder could not be found.'));
    }

    if (!$folder['parent']) {
        return error_response(lang('Sorry, you may not duplicate the root folder.'));
    }
    ...
    log_activity(lang('Folder was duplicated') . ' (' . $folder['name'] . ' -> ' . $new_folder['name'] . ').');
