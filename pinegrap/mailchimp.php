...
function mailchimp_sync_products() {
    ...
    foreach ($product_groups as $product_group) {
        ...
        $existing_product = array_finder($mailchimp_products, 'id', 'pg-' . $product_group['id']);
        ...
        log_activity(lang(array('string'=>'Product Group ({var:1}) was synced with MailChimp.','vars'=>$product_group['name'])) );
    }
    ...
    foreach ($products as $product) {
        ...
        if ($product_group and $product_group['display_type'] == 'browse') {
            ...
            $existing_product = array_finder($mailchimp_products, 'id', 'p-' . $product['id']);
        ...
        foreach ($mailchimp_products as $mailchimp_product) {
            ...
            $existing_variant = array_finder($mailchimp_product['variants'], 'id', $product['id']);
...
function mailchimp_sync_orders() {
    ...
    $self_gift_field = array_finder($merge_fields, 'tag', 'SELF_GIFT');
    ...
    foreach ($orders as $order) {
        ...
        foreach ($items as $item) {
            ...
            if ($product_group['display_type'] == 'select') {
                ...
                $mailchimp_product = array_finder($mailchimp_products, 'id', 'pg-' . $product_group['id']);
                ...
                if ($mailchimp_product) {
                    ...
                    $mailchimp_variant =
                        array_finder($mailchimp_product['variants'], 'id', $product['id']);
            ...
            if (!$mailchimp_product) {
                ...
                $mailchimp_product = array_finder($mailchimp_products, 'id', 'p-' . $product['id']);
        ...
        log_activity(lang(array('string'=>'Order ({var:1}) was synced with MailChimp.','vars'=>$order['order_number'])) );
...
function mailchimp_request($request) {
    ...
    if ($curl_errno) {
        ...
        $message = 
            lang('MailChimp Error') . "\n" .
            lang('cURL Error Number') . ': ' . $curl_errno . '.' . "\n" .
            lang('cURL Error Message') . ': ' . $curl_error . '.' . "\n" .
            lang('Request') . ': ' . print_r($request, true);
    ...
    if ($response_code < 200 or $response_code > 299) {
        ...
        $message = 
            lang('MailChimp Error') . ': ' . $response['title'] . '. ' . $response['detail'] . "\n" .
            lang('Response') . ': ' . print_r($response, true) . "\n" .
            lang('Request') . ': ' . print_r($request, true);
...
function mailchimp_email_error($message) {
    ...
    email(array(
        ...
        'subject' => HOSTNAME_SETTING . ': '. lang('MailChimp Error'),
        'body' => $message));
}