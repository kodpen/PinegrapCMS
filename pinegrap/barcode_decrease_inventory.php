...
if ( !isset( $_GET['request'] ) && empty( $_GET['request'] ) && ($_GET['request'] ?? '') != 'action'){
    ...
    pg_page_shell(
        array(
            'title'=> lang('Product Inventory Quantity Decrease'),
                            ...
                            <table class="table-hover table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="align-middle text-center">' . lang('Deducted Quantity') . '</th>
                ...
                function request_barcode(barcode,quantity){
                    ...
                    if(barcode != "" && quantity != "" && quantity >= 1){
                        $.ajax({
                            ...
                            url: "barcode_decrease_inventory.php?request=action",
                            data: JSON.stringify({
                            ...
                            success: function(response) {
                                if(response.status == "success"){
                                    ...
                                    $("#action_logs tbody").prepend("<tr>\
                                        ...
                                        <td class=\'align-middle text-center text-danger fw-bolder\'>" + response.data.quantity + "</td>\
...
// else this is a ajax request
    ...
    switch ($action) {
        case 'update_inventory_quantity':
            ...
            if($new_inventory_quantity == 0 && $inventory == 1){
                $sql_out_of_stock = "out_of_stock = '1',";
                $sql_out_of_stock_timestamp = "out_of_stock_timestamp = UNIX_TIMESTAMP(),";
            }
