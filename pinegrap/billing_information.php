...
if ($liveform->check_form_errors() == false) {
    ...
    if (!USER_LOGGED_IN or $ghost or ((isset($_SESSION['software']['update_contact']) ? $_SESSION['software']['update_contact'] : null) !== false)) {
        ...
        // Only update tax fields if they were actually present in the submitted form.
        // If the layout didn't include these fields we must not overwrite existing contact data.
        $sql_tax = '';
        if ($liveform->field_in_session('tax_number') || $liveform->field_in_session('tax_office')) {
            $sql_tax = "tax_number = '" . escape(trim($liveform->get_field_value('tax_number'))) . "', " .
