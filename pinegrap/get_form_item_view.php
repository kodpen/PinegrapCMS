...
function get_form_item_view($properties) {
    ...
    if (
        ...
        $output =
            '<p class="software_notice">' . lang('This page will only show data if a reference code is passed to it via the link to this page (typically from a Form List View page).') . '</p>' .
            $layout;
    ...
    // else the user did not come from the control panel, so prepare output differently
        ...
        if (($editable == false) && (mysqli_num_rows($result) == 0)) {
            ...
            // Otherwise if a pretty URL is being used to access this page, then output error for that.
                ...
                output_error(lang('The submitted form could not be found. Please check that you have entered the correct address. The address for the submitted form might have changed, or the submitted form might have been deleted.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.', 404);

            // Otherwise this page is being called through an ugly URL, so output error for that.
            } else {
                if ($_GET['r']) {
                    output_error(lang('The submitted form could not be found. The reference code might be invalid or the submitted form might have been deleted.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.', 404);
                } else {
                    output_error(lang('There was no reference code passed to this page, so a submitted form could not be found. You may browse to a submitted form from a form list view page.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.', 404);
                }
            }
        }
        ...
        if ($submitter_security == 1) {
            ...
            if ($view_access == FALSE) {
                log_activity(lang(array('string'=>'access denied to view submitted form ({var:1}) on form item view because submitter security is enabled and visitor does not have access','vars'=>$submitted_form['reference_code'])), $_SESSION['sessionusername']);
                output_error(lang('Sorry, you do not have access to view this submitted form.') . ' <a href="javascript:history.go(-1)">' . lang('Go back') . '</a>.');
            }
        }
        ...
        if (($edit_access == true) && ((isset($_GET['edit_submitted_form'])) && ($_GET['edit_submitted_form'] == 'true'))) {
                    ...
                    switch ($field['type']) {
                        ...
                        case 'file upload':
                            ...
                            if ($file['name'] != '') {
                                ...
                                if ($field['required'] == 0) {
                                    $output_delete_option = '&nbsp; &nbsp;<label for="' . $field['id'] . '_delete_file">' . lang('or Delete File') . ': </label>' . $liveform->output_field(array('type'=>'checkbox', 'name'=> $field['id'] . '_delete_file', 'id'=> $field['id'] . '_delete_file', 'value'=>'1', 'class'=>'software_input_checkbox'));
                                } else {
                            ...
                            $output_fields .=
                                '<tr' . $row_class . '>
                                    ...
                                    <td style="vertical-align: top">
                                        ...
                                        <script>
                                            software_$("#' . $field['id'] . '").datepicker(datetimepicker_options);
                                        </script>
                                    </td>
                                </tr>';
                            break;
            ...
            if (!$submitted_form['complete']) {
                $output_buttons =
                    '<button type="submit" name="save_for_later_button" value="Save for Later" class="software_input_submit_secondary">' . lang('Save for Later') . '</button>&nbsp;&nbsp;
                    <button type="submit" name="complete_button" value="Complete" class="software_input_submit_primary save_button">' . lang('Complete') . '</button>&nbsp;&nbsp;';

            // Otherwise the form is complete, so show different buttons.
            } else {
                $output_buttons =
                    '<button type="submit" name="save_button" value="Save" class="software_input_submit_primary save_button">' . lang('Save') . '</button>&nbsp;&nbsp;
                    <button type="submit" name="incomplete_button" value="Incomplete" class="software_input_submit_secondary">' . lang('Incomplete') . '</button>&nbsp;&nbsp;';
            }
            
            // if the user has access to delete the submitted form, then output delete button
            if ($delete_access == TRUE) {
                // Example code desc.: Standardize confirmation via native browser confirm modal
                $output_delete_button = '&nbsp;&nbsp;<button type="submit" name="delete_button" value="Delete" onclick="return confirm(\'' . addslashes(lang(array('string'=>'WARNING: This {var:1} will be permanently deleted.','vars'=>array(lang('submitted form'))))) . '\');" class="software_input_submit_secondary delete_button">' . lang('Delete') . '</button>';
            }
            ...
            $output = 
                ...
                <form' . $output_enctype . ' action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_submitted_form.php" method="post" style="margin: 0px">
                    ...
                    <div>
                        ' . $output_buttons . '
                        <button type="button" name="cancel" value="Cancel" onclick="javascript:history.go(-1);" class="software_input_submit_secondary cancel_button">' . lang('Cancel') . '</button>
        ...
        // else user has not chosen to edit submitted form, so just output layout with values
            ...
            preg_match_all('/\{if\s*(.*?\^\^(.*?)\^\^.*?)?(\s*(.*?)\s*)?(\'(.*?)\')?(\s*\}(.*?))?(\{\s*else\s*\}(.*?))?\{\s*endif\s*\}/si', $output, $operators, PREG_SET_ORDER);
            ...
            foreach ($operators as $operator) {
                ...
                if ($field_name_valid == TRUE) {
                    ...
                    switch($operator_string){
                        case "=":
                        case "==":
                            if($submitted_form[$select_name] == $secondary_field_val){
                        ...
                        default:
                            ...
                            $output = str_replace($whole_string, '', $output);
                            break;
                    }                   
                }   
            }
            ...
            if ($layout_type == 'system') {
                ...
                if (!empty($_SESSION['software']['custom_form_auto_registration'][$submitted_form['id']]['email_address'])) {
                    $output_auto_registration =
                        ...
                        <div class="account data">
                            ...
                            <p>
                                ' . lang('Email') . ': ' . h($_SESSION['software']['custom_form_auto_registration'][$submitted_form['id']]['email_address']) . '<br>
                                ' . lang('Password') . ': ' . h($_SESSION['software']['custom_form_auto_registration'][$submitted_form['id']]['password']) . '
                            </p>
                        </div>';
                }
                ...
                if ($form_id == '') {
                    ...
                    if ($edit_access == true) {
                        ...
                        $output_edit_button = '<a href="' . h($edit_url) . '" class="software_button_primary">' . lang('Edit') . '</a>';
                    }
    ...
    if ($editable) {
        ...
        $output =
            '<div class="edit_mode" style="position: relative; outline: 1px dashed #805FA7; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_form_item_view.php?page_id=' . $page_id . '&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button forms" title="' . lang('Form Item View') . ': '. h(get_page_name($page_id)) .'"><i class="bi bi-pencil"></i></a>
            ' . $output . '
            </div>';
    }
