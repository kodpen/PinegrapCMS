...
function get_page_content($page_id, $system_content = '', $extra_system_content = '', $mode = 'preview', $email = false, $dynamic_properties = array(), $toolbar = FALSE, $device_type = 'desktop') {
	...
	$edit_label = '<i class="bi bi-pencil"></i>';
    ...
    // Inject per-page custom fonts / CSS / JS (Visual Pinegrap Editor assets)
    if ($style_type == 'system') {
        // ── Per-page assets: Fonts → CSS (head) | JS (body bottom) ──────────────
        // Note: $page_custom_* variables were captured from the PAGE query above
        // (before $row was overwritten by the style query).

        // 1. FONTS — first in </head>. Loads Google Font <link> tags only.
        // We INTENTIONALLY do not emit a `body { font-family: 'X' }` rule
        // for enabled fonts: that auto-overlay used to silently override
        // the user's design when multiple fonts were enabled (last-wins
        // cascade), and there's no way to "uninstall" the rule short of
        // disabling the font itself. Applying a font to elements is the
        // user's job — via the Options panel font picker (writes
        // tree.props.fontFamily), User Styles, or the Themes panel's
        // `--bs-body-font-family` variable. Loading the link only makes
        // the typeface available; the cascade decides what uses it.
        $pg_pending_overlays_css = '';  // kept for backward-compat below; never written to
        if ($page_custom_fonts !== '') {
            $_font_links = '';
            $_fonts_arr = json_decode($page_custom_fonts, true);
            if (is_array($_fonts_arr)) {
                // New JSON format
                foreach ($_fonts_arr as $_gf) {
                    if (isset($_gf['enabled']) && $_gf['enabled'] === false) continue;
                    if (empty($_gf['family'])) continue;
                    $_wts = !empty($_gf['weights']) ? implode(';', $_gf['weights']) : '400';
                    $_gf_url = 'https://fonts.googleapis.com/css2?family=' . rawurlencode($_gf['family']) . ':wght@' . $_wts . '&display=swap';
                    $_font_links .= '<link rel="stylesheet" href="' . htmlspecialchars($_gf_url, ENT_QUOTES, 'UTF-8') . '">' . "\n";
                }
            } else {
                // Legacy: URL-per-line format
                foreach (explode("\n", $page_custom_fonts) as $_fl) {
                    $_fl = trim($_fl);
                    if ($_fl === '') continue;
                    if (strpos($_fl, '@import') === 0) {
                        $_font_links .= $_fl . "\n";
                    } elseif (filter_var($_fl, FILTER_VALIDATE_URL)) {
                        $_font_links .= '<link rel="stylesheet" href="' . htmlspecialchars($_fl, ENT_QUOTES, 'UTF-8') . '">' . "\n";
                    }
                }
            }
            if ($_font_links !== '') {
                $content = str_replace('</head>', $_font_links . '</head>', $content);
            }
        }

        // ── Bootstrap sentinel CDN URLs ─────────────────────────────────────
        // Single source of truth — Visual Pinegrap Editor entries with
        ...
        // (further down in this function) that the per-page JSON already owns
        // Bootstrap placement, so the legacy injection MUST be suppressed —
        // otherwise Bootstrap would be loaded twice and the user's chosen
        // ordering would be undone.
        $pg_has_bs_css_sentinel = false;
        $pg_has_bs_js_sentinel  = false;

        // Custom CSS: JSON [{id,name,type,content,enabled,locked,meta}] or legacy plain text
        // types: 'css' (inline), 'external-css' (<link href>)
        // meta:  'bootstrap-css' / 'bootstrap-icons' — framework sentinels.
        //        'user-styles'                       — handwritten Styles panel rules.
        $pg_user_styles_css = '';   // collected separately; emitted as its own <style id="pg-user-styles">
        if ($page_custom_css !== '') {
            $_css_inline = '';
            $_css_links  = '';
            $_css_arr = json_decode($page_custom_css, true);
            if (is_array($_css_arr)) {
                foreach ($_css_arr as $_cf) {
                    if (isset($_cf['enabled']) && $_cf['enabled'] === false) continue;
                    $_ctype = isset($_cf['type']) ? $_cf['type'] : 'css';
                    ...
                    if ($_meta === 'bootstrap-icons') {
                        $_css_links .= '<link rel="stylesheet" href="' . htmlspecialchars($_bs_icons_url, ENT_QUOTES, 'UTF-8') . '">' . "\n";
                        $pg_has_bs_css_sentinel = true;
                        continue;
                    }
                    if ($_meta === 'user-styles') {
                        // Stash; emitted last so it cascades over everything
                        // else (page-css, page-overlays).
                        if (!empty($_cf['content'])) $pg_user_styles_css .= $_cf['content'] . "\n";
                        continue;
                    }

                    if ($_ctype === 'external-css') {
                        $_url = trim(isset($_cf['content']) ? $_cf['content'] : '');
                        // Accept absolute URLs (http/https) and root-relative URLs (/) —
                        // OUTPUT_PATH is a root-relative path like "/" so file picker URLs
                        // come back as "/filename.css" which filter_var rejects.
                        if ($_url !== '' && (filter_var($_url, FILTER_VALIDATE_URL) || strpos($_url, '/') === 0)) {
                            $_css_links .= '<link rel="stylesheet" href="' . htmlspecialchars($_url, ENT_QUOTES, 'UTF-8') . '">' . "\n";
                        }
                    } else {
                        // inline css or unknown
                        if (!empty($_cf['content'])) {
                            $_css_inline .= '/* ' . str_replace('*/', '', isset($_cf['name']) ? $_cf['name'] : 'file') . " */\n" . $_cf['content'] . "\n";
                        }
                    }
                }
            } else {
                $_css_inline = $page_custom_css; // legacy plain text
            }
            $_head_inject = '';
            // CRITICAL: sentinels and external links are emitted in their JSON
            // order — concatenate $_css_links FIRST so a user file that was
            // moved above bootstrap-css actually loads first. Inline <style>
            // always lands last so it can override link-loaded rules.
            if ($_css_links  !== '') $_head_inject .= $_css_links;
            if (trim($_css_inline) !== '') $_head_inject .= '<style id="pg-page-css">' . "\n" . $_css_inline . '</style>' . "\n";
            if ($_head_inject !== '') {
                $content = str_replace('</head>', $_head_inject . '</head>', $content);
            }
        }

        // Pending overlay rules — Google Font auto-overlays only. Emitted as
        // <style id="pg-page-overlays"> AFTER pg-page-css so overlay rules
        // cascade over Bootstrap, the user's stylesheets, and the page-css
        // inline block.
        if ($pg_pending_overlays_css !== '') {
            $content = str_replace('</head>',
                '<style id="pg-page-overlays">' . "\n" . $pg_pending_overlays_css . '</style>' . "\n</head>",
                $content);
        }

        // User Styles — handwritten rules from the bottom Styles panel.
        // Emitted as <style id="pg-user-styles"> AFTER pg-page-overlays so
        // they win the cascade against everything else, and kept in their
        // own block so the editor's matching list can identify and skip
        // them (the User Styles section already exposes them editably).
        if ($pg_user_styles_css !== '') {
            $content = str_replace('</head>',
                '<style id="pg-user-styles">' . "\n" . $pg_user_styles_css . '</style>' . "\n</head>",
                $content);
        }

        // Custom JS: JSON [{id,name,type,content,enabled,locked,meta}] or legacy plain text
        // types: 'js' (inline), 'json' (inline as application/json), 'external-js' (<script src>)
        // meta:  'bootstrap-js' — sentinel row that emits the bundle CDN script
        //        at its JSON-array position (default: end of body, can be
        //        moved earlier so it executes before user scripts).
        if ($page_custom_js !== '') {
            $_js_inline  = '';
            $_js_srcs    = '';
            ...
            if (is_array($_js_arr)) {
                foreach ($_js_arr as $_jf) {
                    if (isset($_jf['enabled']) && $_jf['enabled'] === false) continue;
                    $_jtype = isset($_jf['type']) ? $_jf['type'] : 'js';
                    ...
                    if ($_meta === 'jquery-js') {
                        // jQuery sentinel — opt-in (default disabled in the
                        // designer); when enabled we emit the CDN <script>
                        // at the user-chosen position. No legacy injection
                        // path to suppress (jQuery was never auto-included
                        // server-side; pageSettings.jquery was canvas-only).
                        $_js_srcs .= '<script src="' . htmlspecialchars($_jquery_url, ENT_QUOTES, 'UTF-8') . '"></script>' . "\n";
                        continue;
                    }

                    if ($_jtype === 'external-js') {
                        $_url = trim(isset($_jf['content']) ? $_jf['content'] : '');
                        // Accept absolute URLs (http/https) and root-relative URLs (/) —
                        // OUTPUT_PATH is a root-relative path like "/" so file picker URLs
                        ...
                        if (!empty($_jf['content'])) {
                            $_js_inline .= '/* ' . str_replace('*/', '', isset($_jf['name']) ? $_jf['name'] : 'file') . " */\n" . $_jf['content'] . "\n";
                        }
                    }
                }
            } else {
                $_js_inline = $page_custom_js; // legacy plain text
            }
            $_body_inject = '';
    ...
    // ── Shared Component placeholder expansion (F9) ─────────────────────────────
    // style_code is a CACHE, and shared components are referenced by marker (not inline-
    // expanded), so we must resolve `<!--pg-shared-ref:ID-->` markers from the live
    // `shared_components.tree_json` on every page render. This is what makes a shared
    // component edit visible on every referencing style immediately — without rebuilding
    // any style_code. See `_expand_shared_refs()` in functions.php.
    //
    // Run BEFORE cregion/dregion/PHP replacement so a shared component can legitimately
    // contain regions and they get processed by the pipeline below.
    if (strpos($content, '<!--pg-shared-ref:') !== false) {
        $content = _expand_shared_refs($content);
    }

    // ── System widget marker expansion ──────────────────────────────────────────
    // `_expand_shared_refs()` defers system widgets (shared components with
    // system_region_config) to `<!--pg-system-widget:ID-->` markers. Resolve them
    ...
    // Visual-designer `custom_php` content nodes are emitted as `<!--pg-custom-php:BASE64-->`
    // markers. Eval them here so the PHP runs at page-render time (fresh state every request).
    // Runs AFTER shared_refs expansion so PHP snippets living INSIDE a shared component also
    // get executed (shared expansion may splice in new markers from shared_components.tree_json).
    if (strpos($content, '<!--pg-custom-php:') !== false) {
        $content = _expand_custom_php($content);
    }

    // ── Messages placeholder expansion ─────────────────────────────────────────
    // Visual-designer `messages` content nodes baked into style_code as
    ...
    if (strpos($content, '<!--pg-messages-placeholder-->') !== false) {
        ...
        if (!empty($_SESSION['software']['liveforms']) && is_array($_SESSION['software']['liveforms'])) {
            ...
            // / software_warning + alert alert-warning). We tried calling
            // the liveform helpers directly but the instance-based path
            // silently returned empty in some session states; reading the
            // session straight is bulletproof.
            //
            // Pinegrap convention: messages persist until a page handler
            // explicitly calls $liveform->remove_form(). READ ONLY here;
            // never mutate / consume.
            foreach ($_SESSION['software']['liveforms'] as $lf_name => $lf_indexes) {
                ...
                foreach ($lf_indexes as $lf_idx => $lf_data) {
                    ...
                    // Errors are stored field-by-field: $lf_data[field]['error']=TRUE,
                    // $lf_data[field]['error_message']='...'. Iterate every entry whose
                    // value is itself an array carrying an active error flag.
                    $err_lines = '';
                    $err_count = 0;
                    ...
                    // Notices and warnings are plain string arrays at the index root.
        ...
        // Replace ONLY the first marker with the messages HTML — system widgets
        // (catalog_item_view, catalog_listing) auto-prepend a marker so server-
        // side validation errors (e.g. "Lütfen bir alıcı seçin") surface on
        // the redirected page even when the designer didn't manually place a
        // messages region. Stripping the remaining markers prevents the same
        // alert from showing up multiple times when several widgets sit on
        // the same page.
        $content = preg_replace('/<!--pg-messages-placeholder-->/', $pg_messages_html, $content, 1);
        $content = str_replace('<!--pg-messages-placeholder-->', '', $content);
    }
    ...
    foreach ($regions[0] as $region) {
        ...
        if ($cregion_designer_type == 'yes') {
            ...
            foreach ($embedded_regions[0] as $embedded_region) {
                ...
                if ($embedded_cregion_designer_type == 'yes') {
                    ...
                    if (($mode == 'edit') && ($user['role'] < 2)) {
                        ...
                        $embedded_cregion_content = '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_designer_region.php?id=' . $embedded_cregion_id . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button design" title="' . lang('Designer Region') . ': ' . h($embedded_cregion_name) . '">' . $edit_label . '</a>'. $embedded_cregion_content . '</div>';
                    }
            ...
            if (($mode == 'edit') && ($user['role'] < 2)) {
                ...
                $cregion_content = '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_designer_region.php?id=' . $cregion_id . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button design" title="' . lang('Designer Region') . ': ' . h($cregion_name) . '">' . $edit_label . '</a>'. $cregion_content . '</div>';
            }
    ...
    // Otherwise figure out the path and query string.
        ...
        switch ($page_type) {
            ...
            case 'form list view':
                ...
                if (isset($_GET[$page_id . '_page_number']) && $_GET[$page_id . '_page_number'] != '' and $_GET[$page_id . '_page_number'] > 1) {
                ...
                } else if (isset($_GET['page_number']) && $_GET['page_number'] != '' and $_GET['page_number'] > 1) {
    ...
    if ($email == FALSE) {
        ...
        if ($mode == 'edit') {
            $output_edit_region_code .=
                ...
                <style type="text/css">
                    ...
                    {
                        color: #fff !important;
                        background: #3e6b0e url(' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/submit_primary_background.png) top repeat-x !important;
                    ...
                    {
                        color: #000 !important;
                        background: #d1d1d1 url(' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/submit_secondary_background.png) top repeat-x !important;
        ...
        $output_restricted_area_translates = '';
        if(isset($_SESSION['sessionusername']) != false){
            $output_restricted_area_translates = '
                "Deactivate Fullscreen Mode":"' . lang('Deactivate Fullscreen Mode') . '",
                "Activate Fullscreen Mode":"' . lang('Activate Fullscreen Mode') . '",
                "Page Region":"' . lang('Page Region') . '",
                "Common Region":"' . lang('Common Region') . '",
                "System Region Header":"' . lang('System Region Header') . '",
                "System Region Footer":"' . lang('System Region Footer') . '",
                "Your updates have been <strong>canceled</strong>":"' . lang('Your updates have been <strong>canceled</strong>') . '",
                "Your updates have been <strong>saved</strong>.":"' . lang('Your updates have been <strong>saved</strong>.') . '",
                "Software Image Editor":"' . lang('Software Image Editor') . '",
                ...
                "Saving... (please wait)":"' . lang('Saving... (please wait)') . '",
        ...
        <script>
            ...
            var translate ={
                "Sorry, we don\'t have any verified addresses for that Country.":"' . lang('Sorry, we don\'t have any verified addresses for that Country.') . '",
                "Sorry, we don\'t have any verified addresses for that State.":"' . lang('Sorry, we don\'t have any verified addresses for that State.') . '",
                "The shipping address fields have been updated with":"' . lang('The shipping address fields have been updated with') . '",
                "Remove Advanced Search":"' . lang('Remove Advanced Search') . '",
                "Add Advanced Search":"' . lang('Add Advanced Search') . '",
                "Attribute Helper":"' . lang('Attribute Helper') . '",
                "Copy to Clipboard":"' . lang('Copy to Clipboard') . '",
                "Cancel":"' . lang('Cancel') . '",
                "Notice":"' . lang('Notice') . '",
                "Error":"' . lang('Error') . '",
                "An error occurred":"' . lang('An error occurred') . '",
                "Rich-text Editor":"' . lang('Rich-text Editor') . '",
                "Share":"' . lang('Share') . '",
        ...
        if ((GOOGLE_ANALYTICS == TRUE) && (GOOGLE_ANALYTICS_WEB_PROPERTY_ID != '')) {
            ...
            if (
                ...
                foreach ($order_items as $order_item) {
                    ...
                    $output_ecommerce_tracking_data .=
                        '            {' . "\n" .
                        '                item_id: \'' . escape_javascript($order_item['product_name']) . '\',' . "\n" .
                        '                item_name: \'' . escape_javascript($order_item['short_description']) . '\',' . "\n" .
            ...
            $output_google_analytics =
                '<!-- Google tag (gtag.js) -->' . "\n" .
                '<script async src="https://www.googletagmanager.com/gtag/js?id=' . GOOGLE_ANALYTICS_WEB_PROPERTY_ID . '"></script>' . "\n" .
                '<script>' . "\n" .
                '    window.dataLayer = window.dataLayer || [];' . "\n" .
                '    function gtag(){dataLayer.push(arguments);}' . "\n" .
                '    gtag(\'js\', new Date());' . "\n" .
                '    gtag(\'config\', \'' . GOOGLE_ANALYTICS_WEB_PROPERTY_ID . '\');' . "\n" .
                    $output_ecommerce_tracking_data .
                '</script>';
            ...
            $output_text_editor_content = 
                '<div id="software_edit_region_dialog">
                    <form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/save_region_content.php" method="post">
                        ...
                        <div><button type="submit" name="submit" value="Save" class="submit-primary">' . lang('Save') . '</button>&nbsp;&nbsp;&nbsp;<input type="button" name="cancel" value="Cancel" onclick="software_$(software_$(\'#software_edit_region_dialog\')[0]).dialog(\'close\')" class="submit-secondary"></div>
                    </form>
                </div>
        ...
        if (mb_strpos(mb_strtolower($tag[1]), 'view-access') !== false) {
            ...
            if ($folder_id) {
                ...
                // then output error.
                    ...
                    $if_content ='<div style="color:red;">' . lang(array('string'=>'Sorry, a folder could not be found for the folder id ("{var:1}") in the if tag. An incorrect folder id might have been entered or the folder might have been deleted since the if tag was added. Please use the following format: {var:2}','vars'=>array(h($original_folder_id), h('<if view-access folder-id="123">' . lang('content') . '</if>') ) )) . '</div>';
                }
                
            // Otherwise a folder id was not found, so if the user is in edit mode,
            // then output error.
            } else if ($mode == 'edit') {
                $if_content ='<div style="color:red;">' . lang(array('string'=>'Sorry, the if tag is missing a folder id. Please use the following format: {var:1}','vars'=>h('<if view-access folder-id="123">' . lang('content') . '</if>') )) . '</div>';
            }
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if ($ad_region_display_type == 'static') {
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        if (($mode == 'edit') && (($user['role'] < 3) || (in_array($ad_region_id, get_items_user_can_edit('ad_regions', $user['id'])) == true))) {
                            ...
                            $ad_content =
                                '<div class="edit_mode" style="position: relative; outline: 1px dashed #788207;">
                                    <a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_ad.php?id=' . $ad_id . '&send_to=' . h(urlencode(get_request_uri())) . '"class="software_pinegrap_inline_edit_button ads" title="' . lang('Ad') . ': ' . h($ad_name) . '">' . $edit_label . '</a>
                                    ' . $ad_content . '
                                </div>';
                        }
                    }
                ...
                // else this is a dynamic ad region, so prepare content for dynamic ad region
                    ...
                    if (mysqli_num_rows($result) > 0) {
                            ...
                            if (($mode == 'edit') && (($user['role'] < 3) || (in_array($ad_region_id, get_items_user_can_edit('ad_regions', $user['id'])) == true))) {
                                ...
                                $ad['content'] =
                                    '<div style="border: 1px dashed #788207">
                                        <a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_ad.php?id=' . $ad['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '"class="software_pinegrap_inline_edit_button ads" title="' . lang('Ad') . ': ' . h($ad['name']) . '">' . $edit_label . '</a>
                                        ' . $ad['content'] . '
                                    </div>';
                            }
                        ...
                        if (count($ads) > 1) {
                            $output_previous_and_next_buttons =
                                '<a class="previous" title="' . lang('Previous') . '"></a>
                                <a class="next" title="' . lang('Next') . '"></a>';
                        }
                ...
                if (($mode == 'edit') && (($user['role'] < 3) || (in_array($ad_region_id, get_items_user_can_edit('ad_regions', $user['id'])) == true))) {
                    $ad_region_content =
                        '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;">
                            <a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_ad_region.php?id=' . $ad_region_id . '&send_to=' . h(urlencode(get_request_uri())) . '"class="software_pinegrap_inline_edit_button design" title="' . lang('Ad Region') . ': ' . h($ad_region_name) . '">' . $edit_label . '</a>
    ...
    if ((ECOMMERCE == true) && (preg_match('/<cart><\/cart>/i', $content) != 0)) {
        ...
        if ($number_of_items > 0) {
            ...
            $item_label =lang(array('string'=>'Item{suffix:1}','suffix'=>$plural_suffix ));
    ...
    foreach ($regions[0] as $region) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $menu_item_edit_context = ($mode == 'edit') ? array('from' => 'pages', 'send_to' => get_request_uri()) : array();
            $menu_content .= get_menu_content($menu_id, 0, $current_menu_item_id, ($mode == 'edit'), $menu_item_edit_context);
            ...
            if (($mode == 'edit') && (($user['role'] < 3) || (in_array($menu_id, get_items_user_can_edit('menus', $user['id'])) == true))) {
                $menu_content =
                    '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;">
                        <a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/view_menu_items.php?id=' . $menu_id . '&from=pages&send_to=' . h(urlencode(get_request_uri())) . '"class="software_pinegrap_inline_edit_button design" title="' . lang('Menu') . ': ' . h($menu_name) . '">' . $edit_label . '</a>
                        ' . $menu_content . '
                    </div>';
            }
        }
        ...
        if (MOBILE == true) {
            ...
            switch ($requested_device_type) {
                ...
                default:
                    ...
                    $output_mobile_switch_label = lang('Mobile Site');
                ...
                case 'mobile':
                    ...
                    $output_mobile_switch_label = lang('Full Site');
    ...
    if (preg_match('/<pdf><\/pdf>/i', $content) != 0) {
        ...
        $pdf_link = '<div class="software_pdf_link"><a href="' . URL_SCHEME . HOSTNAME . h($parsed_url['path'] . $query_string) . '" target="_blank"><img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/icon_pdf.gif" width="55" height="20" alt="' . lang('PDF Version') . '" title="' . lang('PDF Version') . '" border="0" /></a></div>';
    ...
    // ── System-widget catalog RSS + structured data (JSON-LD) ──────────────────
    // Legacy RSS <link> autodiscovery and schema.org JSON-LD are only produced by
    // the whole-page-type-driven pipeline further below (page_type 'catalog' /
    // 'catalog detail', etc.) — a page built with the visual designer's "Katalog
    // Ürünleri" (catalog_listing) / "Ürün Detay" (catalog_item_view) system
    // widgets is page_type='standard' and never matches that gate, so it got
    // neither feature. This block extends both to those pages, using the exact
    // same product data + resolution rules the widget itself renders with (see
    // functions.php: _pg_find_system_widget_on_page and friends). Kept
    // independent of the per-system-region loop below rather than woven into
    // it, since that loop's shape assumes a legacy whole-page system region.
    if ($mode != 'edit' && $page_type == 'standard') {
        ...
        if ($_pg_sw_access_control_type == 'public') {
            ...
            $_pg_sw_page_full  = isset($_GET['page']) ? (string)$_GET['page'] : '';
            $_pg_sw_slash      = mb_strpos($_pg_sw_page_full, '/');
            $_pg_sw_sub_path   = ($_pg_sw_slash !== false) ? mb_substr($_pg_sw_page_full, $_pg_sw_slash + 1) : '';
            ...
            if ($_pg_sw_listing_widget) {
                ...
                if ($_pg_sw_structured_data_enabled) {
                    ...
                    $_pg_sw_products = _pg_catalog_listing_rss_products($_pg_sw_active_group, 'sort_order', 'DESC', 60);
                    if (is_array($_pg_sw_products) && count($_pg_sw_products) > 0) {
                        $_pg_sw_detail_page_id = isset($_pg_sw_listing_widget['cfg']['detail_page_id']) ? (int)$_pg_sw_listing_widget['cfg']['detail_page_id'] : 0;
                        $_pg_sw_detail_page_name = ($_pg_sw_detail_page_id > 0)
                            ? (string)db_value("SELECT page_name FROM page WHERE page_id = '" . $_pg_sw_detail_page_id . "' LIMIT 1")
                            : '';
                        $_pg_sw_jsonld = _pg_build_catalog_listing_jsonld($_pg_sw_products, $page_name, $_pg_sw_detail_page_name);
                        $content = preg_replace(
                            '/<\/head>/i',
                            "\n<!-- Start Structured Data -->\n<script type=\"application/ld+json\">" . json_encode($_pg_sw_jsonld, JSON_UNESCAPED_UNICODE) . "</script>\n<!-- End Structured Data -->\n</head>",
                            $content,
                            1
                        );
                    }
    ...
    // Detect if the style has a comments_block placeholder.
    // When present, comments HTML is injected at the placeholder position
    // instead of being appended inside the system region output.
    $has_comments_block = (strpos($content, '<!--pg-comments-block-->') !== false);
    $comments_block_html = '';

    // If the style has a Comments Block but NO <system> region anywhere,
    // the comments rendering pipeline (which lives inside the foreach
    // below) never fires and the marker silently expands to nothing.
    // Inject a synthetic primary system region right before the marker so
    // the loop populates $comments_block_html. The synth region is wrapped
    // in marker comments so we can nuke its (unwanted) other output
    // after the loop completes — see the matching preg_replace below.
    $injected_synth_system = false;
    if ($has_comments_block
        && strpos($content, '<system>') === false
        && strpos($content, '<system ') === false) {
        $content = str_replace(
            '<!--pg-comments-block-->',
            '<!--pg-synth-system-start--><system></system><!--pg-synth-system-end--><!--pg-comments-block-->',
            $content
        );
        $injected_synth_system = true;
    }
    ...
    foreach ($regions[0] as $region) {
        ...
        if ($system_page_name == '') {
            ...
            $system_region_properties['comments_rating'] = $comments_rating;
        ...
        if (
                ...
                if (($mode == 'edit') && ($edit_access == true)) {
                    if ($page_editor_version == 'latest') {
                        ...
                        $output_system_region_header =
                            '<div class="edit_mode" style="position: relative; outline: 1px dashed black;">
                                <div id="software_inline_editing_region_' . $inline_editing_region_count . '" class="software_system_region_header" title="' . lang('System Region Header') . ': ' . h($system_region_properties['page_name']) . '">
                                    ' . $output_system_region_header . '
                                </div>
                        ...
                        $output_system_region_header =
                            '<div class="edit_mode" style="position: relative; outline: 1px dashed black;">
                                <a href="javascript:void(0)" onclick="software_open_edit_region_dialog(\'' . $system_region_properties['page_id'] . '\', \'system_region_header\', \'\', \'\')" class="software_pinegrap_inline_edit_button system"  title="' . lang('System Region Header') . ': ' . h($system_region_properties['page_name']) . '">' . $edit_label . '</a>
            ...
            if (
                ...
                if ($output_rss_url != '') {
                    ...
                    if ($system_region_properties['page_title'] != '') {
                        ...
                        $output_rss_title = h(HOSTNAME) . ' - [' . lang(array('string'=>'No Web Browser Title property found for {var:1}','vars'=>h($system_region_properties['page_name']) )) . ']';
                    }
            ...
            if (
                ...
                if (SOCIAL_NETWORKING_TYPE == 'simple') {
                    ...
                    if ($output_rss_url != '') {
                        $output_rss_button = '<div style="float: left; margin-right: 14px; padding-top: .5em"><a href="' . $output_rss_url . '"><img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/icon_rss.png" width="27" height="20" alt="RSS" title="' . lang('Subscribe to RSS feed') . '" border="0" /></a></div>';
                    }
                    
                    // Build share buttons using URL-based links (no external SDKs).
                    $encoded_url = rawurlencode($canonical_url);
                    $btn = 'display:inline-block;padding:4px 11px;border-radius:3px;color:#fff;font-family:sans-serif;font-size:13px;text-decoration:none;font-weight:500;';

                    $output_facebook_button = '';
                    if (SOCIAL_NETWORKING_FACEBOOK == TRUE) {
                        $output_facebook_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://www.facebook.com/sharer/sharer.php?u=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#1877f2;">Facebook</a>
                            </div>';
                    }
                    ...
                    if (SOCIAL_NETWORKING_TWITTER == TRUE) {
                        $output_twitter_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://twitter.com/intent/tweet?url=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#000;">X</a>
                            </div>';
                    }
                    ...
                    if (SOCIAL_NETWORKING_LINKEDIN == TRUE) {
                        $output_linkedin_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://www.linkedin.com/sharing/share-offsite/?url=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#0a66c2;">LinkedIn</a>
                            </div>';
                    }

                    $output_whatsapp_button = '';
                    if (SOCIAL_NETWORKING_WHATSAPP == TRUE) {
                        $output_whatsapp_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://wa.me/?text=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#25d366;">WhatsApp</a>
                            </div>';
                    }

                    $output_telegram_button = '';
                    if (SOCIAL_NETWORKING_TELEGRAM == TRUE) {
                        $output_telegram_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://t.me/share/url?url=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#229ed9;">Telegram</a>
                            </div>';
                    }

                    $output_pinterest_button = '';
                    if (SOCIAL_NETWORKING_PINTEREST == TRUE) {
                        $output_pinterest_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://pinterest.com/pin/create/button/?url=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#e60023;">Pinterest</a>
                            </div>';
                    }

                    $output_reddit_button = '';
                    if (SOCIAL_NETWORKING_REDDIT == TRUE) {
                        $output_reddit_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="https://www.reddit.com/submit?url=' . $encoded_url . '" target="_blank" rel="noopener" style="' . $btn . 'background:#ff4500;">Reddit</a>
                            </div>';
                    }

                    $output_email_button = '';
                    if (SOCIAL_NETWORKING_EMAIL == TRUE) {
                        $output_email_button =
                            '<div style="float:left;margin-right:8px;padding-top:.5em;padding-bottom:.5em">
                                <a href="mailto:?body=' . $encoded_url . '" style="' . $btn . 'background:#555;">Email</a>
                            </div>';
                    }
            ...
            switch ($system_region_properties['page_type']) {
                ...
                case 'error':
                    $system_output .= '<div class="software_error_page software_error">';
                        ...
                        // else, assign system output
                            ...
                            $system_output .= '<i>' . lang('Site error messages will appear here.') . '</i>';
                        }
                    $system_output .= '</div>';
                    break;
                ...
                case 'view order':
                    ...
                    $system_output .= 
                        '<div class="software_view_order">
                            ' . get_view_order_screen_content(array(
                                'device_type' => $device_type,
                                'page_id' => $system_region_properties['page_id'])) . '
                        </div>';
                ...
                case 'custom form':
                    ...
                    $pg_custom_form_widget_active = (int) db_value(
                        "SELECT COUNT(*)
                         ...
                         WHERE pr.pregion_page = '" . e($system_region_properties['page_id']) . "'
                ...
                case 'calendar event view':
                    ...
                    if ($mode == 'edit') {
                        ...
                        if (
                            ...
                            $output_calendar_event_view = '<div class="edit_mode" style="position: relative; outline: 1px dashed #FF9539;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_calendar_event.php?id=' . h($_GET['id']) . $output_query_string_recurrence_number . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button calendar" title="' . lang('Calendar Event') . ': ' . h($calendar_event['name']) . '">' . $edit_label . '</a>'. $output_calendar_event_view . '</div>';
                        }
                    }
                    ...
                    if (isset($_GET['page']) && (mb_strpos($_GET['page'], '/') !== FALSE)) {
                        ...
                        if (($mode == 'edit') && (($user['role'] < 3) || ($user['manage_ecommerce'] == true))) {
                            ...
                            if ($item['type'] == 'product group') {
                                ...
                                $output_tooltip_title = lang('Product Group');
                            ...
                            // else the item is a product, so set information in a different way
                                ...
                                // output title for tooltip
                                $output_tooltip_title = lang('Product');
                            }
                            ...
                            $output_catalog_detail = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_link_page . '?id=' . $item['id'] . '&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $output_tooltip_title . ': ' . h($output_tooltip) . '">' . $edit_label . '</a>'. $output_catalog_detail . '</div>';
                        }
                    }
                ...
                case 'order form':
                    ...
                    if (($mode == 'edit') && (($user['role'] < 3) || ($user['manage_ecommerce'] == true))) {
                        ...
                        // add edit container to product group
                        $output_order_form = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_product_group.php?id=' . $row['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . lang('Product Group') . ': ' . h($output_tooltip) . '">' . $edit_label . '</a>'. $output_order_form . '</div>';
                    }
            ...
            if ($system_region_properties['comments'] == '1') {
                ...
                if (count($comments) > 0) {
                    ...
                    // Otherwise all comments are being outputted, so prepare values for that.
                        ...
                        if ($featured_comments_exist == true) {
                            ...
                            $output_switch = '<a href="' . $output_switch_url . '" class="show_featured">' . lang('Show Featured') . '</a> &nbsp; &nbsp; ';
                    ...
                    foreach ($comments as $comment) {
                        ...
                        if (
                            ...
                            $output_published_notices .= lang('Not Published');
                            ...
                            if ($comment['publish_date_and_time'] != '0000-00-00 00:00:00') {
                                ...
                                $output_relative_time = str_replace(lang('A moment'),lang('a moment'), $output_relative_time);

                                $output_published_notices .= ' (' . lang(array('string'=>'scheduled for {var:1}','vars'=>$output_relative_time));

                                if ($comment['publish_cancel']) {
                                    $output_published_notices .= ', ' . lang(array('string'=>'unless a new {var:1} is added first','vars'=>$output_comment_label_lowercase));
                                }
                        ...
                        if (
							...
							$output_published_notices ='';
                            $output_published_notices .= lang(array('string'=>'Your {var:1} is awaiting approval','vars'=>$output_comment_label_lowercase));
                        }
                        
                        // if there is not a comment name to output, then output anonymous
                        if ($comment['name'] == '') {
                            $comment['name'] = lang('Anonymous');
                        }
                        ...
                        //command rating
                        //if rating is not 0 than 
                        $output_comment_rating = '';
                        if($comments_rating != false && $comment['rating'] != 0){
                            $i = 1;
                            $stars = '';
                            //we output 5 star svg icon
                            while($i <= 5){
                                //output filled stars while rating
                                if($comment['rating'] >= $i){
                                    $stars .= '<span class="star-fill"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill" viewBox="0 0 16 16"><path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/></svg></span>';
                                }else{
                                    //else output not unfilled stars.
                                    $stars .= '<span class="star"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star" viewBox="0 0 16 16"><path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"/></svg></span>';
                                }
                                $i++;
                            }
						...
						$output_comments .=
                            ...
                            <div class="comment row_' . ($count % 2) . $output_featured_class . '" style="position: relative">
                                ...
                                ' . $output_comment_rating . '
                                <div class="name_line">
                                    ' . lang(array('string'=>'{var:1} {var:2}','vars'=>array('<span class="added_by">' . lang('Added by') . '</span>','<span class="name">' . h($comment['name']) . '</span>') )) .
                                ...
                                ' . $output_file_attachment . '
                                <a href="javascript:void(0)" class="share" style="position: absolute; top: 1em; right: 1em; padding: .25em" title="' . lang('Share') . ': ' . $output_comment_label . '" data-id="' . $comment['id'] . '">
                                        <img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/share.png" width="20" height="20" alt="' . lang('Share') . '" style="opacity: 0.15">
                                </a>
                            </div>
                            ' . $output_edit_container_end;
                    }
                ...
                if ($edit_access == TRUE) {
                    ...
                    if ($allow_new_comments == TRUE) {
                        $action = 'disallow';
                        $output_allow_new_comments_button_label_prefix = lang(array('string'=>'Do Not Allow New {var:1}','vars'=>h(get_comment_label(array('label' => $system_region_properties['comments_label'],'number' => 2)))));
                       
                    // else new comments are not allowed, so prepare action and prepare to output allow button
                    } else {
                        $action = 'allow';
                        $output_allow_new_comments_button_label_prefix = lang(array('string'=>'Allow New {var:1}','vars'=>h(get_comment_label(array('label' => $system_region_properties['comments_label'],'number' => 2)))));
                    }
                ...
                if ($allow_new_comments == TRUE) {
                    ...
                    // else prepare and output the comment form
                        ...
                        if (
                            ...
                            $output_name_optional_label = lang(' (optional)');
                        ...
                        if ($edit_access || $form_editor) {
							...
							$publish_options[lang('Now')] = 'now';
							$publish_options[lang('At a Scheduled Time')] = 'schedule';
							$publish_options[lang('Maybe Later')] = 'manual';
                            ...
                            $publish_label = lang('Publish') . ': ';
                            $publish_cancel_label = lang(array('string'=>'Cancel if a new {var:1} is added first.','vars'=>$output_comment_label_lowercase));
                        ...
                        $added_form_by_label =lang('Your Name');
                        $comment_add_button_value = lang(array('string'=>'Add {var:1}','vars'=>$output_comment_label));
                ...
                if ($system_region_properties['comments_watcher_email_page_id'] != 0) {
                    ...
                    if (($number_of_watchers == 1) && ($visitor_is_a_watcher == FALSE)) {
                        $output_watcher_count = '<div class="watcher_count">' . lang(array('string'=>'{var:1} {var:2} will be notified when a {var:3} is added.','vars'=>array('1',lang('person'),$output_comment_label_lowercase))) . '</div>';
                    ...
                    // Otherwise the user does not have access to manage watchers, so output standard watcher area.
                        ...
                        if (
                            ...
                            if (
								...
								$output_watcher_question = '<div class="watcher_question">' . lang(array('string'=>'Would you like to be notified when a {var:1} is added?','vars'=>$output_comment_label_lowercase)) . '</div>';
                            // else if the visitor is logged in and he/she is a watcher, then prepare to output question in a different way
								...
								$output_watcher_question = '<div class="watcher_question">' . lang(array('string'=>'Do you no longer want to be notified when a {var:1} is added?','vars'=>$output_comment_label_lowercase)) . '</div>';
                            }
                            ...
                            if (isset($_SESSION['sessionusername']) == TRUE) {
                                ...
                                if ($visitor_is_a_watcher == TRUE) {
                                    $action = 'remove';
                                    $output_add_or_remove_button_label = lang('Remove Me');
                                    
                                // else the user is not a watcher, so prepare action and prepare to output add button
                                } else {
                                    $action = 'add';
                                    $output_add_or_remove_button_label = lang('Add Me');
                ...
                // output comments
                // If a comments_block placeholder exists in the style, redirect comments
                ...
                if ($has_comments_block && $primary_system_region) {
                    $comments_block_html .= $_comments_html;
                } else {
                    $system_output .= $_comments_html;
                }
                ...
                if (($mode == 'edit') && ($edit_access == true)) {
                    if ($page_editor_version == 'latest') {
                        ...
                        $output_system_region_footer =
                            '<div class="edit_mode" style="position: relative; outline: 1px dashed black;">
                                <div id="software_inline_editing_region_' . $inline_editing_region_count . '" class="software_system_region_footer" title="' . lang('System Region Footer') . ': ' . h($system_region_properties['page_name']) . '">
                                    ' . $output_system_region_footer . '
                                </div>
                        ...
                        $output_system_region_footer =
                            '<div class="edit_mode" style="position: relative; outline: 1px dashed black;">
                                <a href="javascript:void(0)" onclick="software_open_edit_region_dialog(\'' . $system_region_properties['page_id'] . '\', \'system_region_footer\', \'\', \'\')" class="software_pinegrap_inline_edit_button system"  title="' . lang('System Region Footer') . ': ' . h($system_region_properties['page_name']) . '">' . $edit_label . '</a>
            ...
            if (($mode == 'edit') && ($edit_access == true)) {
                // Route to the visual editor for system-style pages, otherwise to edit_page.php
                if (!empty($system_region_properties['layout_type']) && $system_region_properties['layout_type'] === 'system' && !empty($system_region_properties['page_style'])) {
                    $_sr_edit_url = OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_system_style.php?id=' . urlencode($system_region_properties['page_style']) . '&amp;send_to=' . h(urlencode(get_request_uri()));
                } else {
                    $_sr_edit_url = OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_page.php?id=' . $system_region_properties['page_id'] . '&amp;send_to=' . h(urlencode(get_request_uri())) . '#interactive_page_feature';
                }
                $system_output =
                    '<div class="edit_mode" style="border: 1px dashed #000; margin: .25em -1px; position: relative">
                        <a href="' . $_sr_edit_url . '" class="software_pinegrap_inline_edit_button system" title="' . lang('System Region') . ': ' . h($system_region_properties['page_name']) . '">' . $edit_label . '</a>
    ...
    // If a comments_block placeholder was used, replace it with the collected comments HTML.
    // Re-detect AFTER system regions have expanded — system widgets (via
    // _render_system_widget_*) can emit comments_block markers from their own
    // tree (e.g. a catalog_item_view's "Reviews" tab). Those markers don't
    // exist in $content at the initial detection point above (line ~1790)
    // because the <system>...</system> placeholders haven't been replaced
    // yet. Without this re-check, comments inside system widgets render empty.
    if (!$has_comments_block && strpos($content, '<!--pg-comments-block-->') !== false) {
        $has_comments_block = true;
    }
    if ($has_comments_block) {
        $content = str_replace('<!--pg-comments-block-->', $comments_block_html, $content);
    }

    // If we injected a synthetic system region above (because the style had
    // a Comments Block but no real <system> tag), strip the synth wrapper
    // and everything the system pipeline emitted into it. The expanded
    // system output sits BETWEEN our marker comments — including a system
    // header, page title elements, edit buttons etc. — none of which the
    // designer asked for. The Comments Block content was already redirected
    // into $comments_block_html and replaced above, so removing the synth
    // wrapper leaves only the comments at the designer-chosen position.
    if ($injected_synth_system) {
        $content = preg_replace(
            '/<!--pg-synth-system-start-->[\s\S]*?<!--pg-synth-system-end-->/',
            '',
            $content
        );
    }
        ...
        if ($cregion_designer_type == 'no') {
            ...
            if (
                ...
                if ($page_editor_version == 'latest') {
                    ...
                    $cregion_content =
                        '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;">
                            <div id="software_inline_editing_region_' . $inline_editing_region_count . '" title="' . lang('Common Region') . ': ' . h($cregion_name) . '">
                                '. $cregion_content . '
                            </div>
                    ...
                    $cregion_content =
                        '<div class="edit_mode" style="position: relative; outline: 1px dashed #c2185b;">
                            <a href="javascript:void(0)" onclick="software_open_edit_region_dialog(\'' . $cregion_id . '\', \'cregion\', \'' . h(escape_javascript($cregion_name)) . '\', \'\')" class="software_pinegrap_inline_edit_button design" title="' . lang('Common Region') . ': ' . h($cregion_name) . '">' . $edit_label . '</a>
                            '. $cregion_content . '
                        </div>';
                }
            }
        ...
        for ($i = 1; $i <= $number_of_regions; $i++) {
            ...
            if ($mode == 'edit') {
                if ($page_editor_version == 'latest') {
                    ...
                    $pregion_content =
                        '<div class="edit_mode" id="pregion_' . $pregion_id . '" style="position: relative; outline: 1px dashed #4780C5;">
                            <div id="software_inline_editing_region_' . $inline_editing_region_count . '" title="' . lang('Page Region') . ': #'. $i .'">
                                '. $pregion_content . '
                            </div>
                    ...
                    $pregion_content =
                        '<div class="edit_mode" id="pregion_' . $pregion_id . '" style="position: relative; outline: 1px dashed #4780C5;">
                            <a href="javascript:void(0)" onclick="software_open_edit_region_dialog(\'' . $pregion_id . '\', \'pregion\', \'\', \'' . $i . '\')" class="software_pinegrap_inline_edit_button page_region" title="' . lang('Page Region') . ': #'. $i .'">' . $edit_label . '</a>
                            '. $pregion_content . '
                        </div>';
                }
            }
    ...
    if (isset($_GET['edit_theme']) && $_GET['edit_theme'] == 'true') {
        $stylesheet = '';
    ...
    // Otherwise the style has no explicit theme.
    } else {
        // For system styles, do NOT fall back to the globally activated theme —
        // system styles are expected to be self-contained. Leaving $stylesheet
        // empty means no theme CSS is injected.
        if ($style_type == 'system') {
            $stylesheet = '';

        // For custom (and any legacy) styles, preserve the original behavior:
        // fall back to the activated desktop/mobile theme so existing sites
        // that rely on an activated global theme keep rendering correctly.
        } else {
    ...
    // For Visual Pinegrap Editor styles, prepend Bootstrap 5 CSS before the theme stylesheet
    // so the theme can override Bootstrap defaults via cascade.
    // Skip when the page's custom-CSS JSON already contains bootstrap-css /
    ...
    // Check before replacing so we know whether a fallback injection is needed.
    $has_stylesheet_placeholder = (stripos($content, '<stylesheet></stylesheet>') !== false);
    $content = preg_replace('/<stylesheet><\/stylesheet>/i', $stylesheet, $content);

    // Fallback: if no <stylesheet> placeholder was present (e.g. Visual Pinegrap Editor system
    // styles — the placeholder was intentionally removed) but we still have CSS links to
    // inject (Bootstrap, theme override, etc.), insert them before </head>.
    if (!$has_stylesheet_placeholder && $stylesheet !== '') {
        $content = preg_replace('/(<\/head>)/i', $stylesheet . "\n" . '$1', $content, 1);
    }
    ...
    if (!$email) {
        ...
        if (AUTO_DIALOGS) {
            ...
            if (
                (isset($_GET['preview_auto_dialog']) && $_GET['preview_auto_dialog'])
                && USER_LOGGED_IN
                && (USER_ROLE < 3)
            ) {
    ...
    // For Visual Pinegrap Editor styles, inject Bootstrap 5 bundle JS before </body>.
