...
function get_shipping_method($properties) {
    ...
    // else the user did not come from the control panel, so perform validation to make sure that this user has access to the ship to
        ...
        if (!isset($_GET['ship_to_id']) || empty($_GET['ship_to_id'])) {
            $form->mark_error('', lang('We\'re sorry, shipping information can only be gathered during the checkout process. <a href="javascript:history.go(-1)">Go back</a>'));
            $content =
        ...
        if (!$recipient) {
           output_error(lang('Sorry, that recipient could not be found.  This might happen if you used the back button to go back to a recipient that no longer exists in the order.'));
        }
    ...
    if ($layout_type == 'system') {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $output_requested_arrival_date =
                '<td style="vertical-align: top">
                    <div class="arrival heading">' . lang('Requested Arrival Date') . ' &nbsp;<a class="software_button_tiny_secondary" href="' . OUTPUT_PATH . h(encode_url_path(get_page_name($_SESSION['ecommerce']['shipping_address_and_arrival_page_id']))) . '?ship_to_id=' . h(urlencode($_GET['ship_to_id'])) . '">' . lang('Change') . '</a></div>
                    <div class="data">
                        ' . $output_arrival_date . '
                    </div>
                </td>';
        ...
        if (ECOMMERCE_RECIPIENT_MODE == 'multi-recipient') {
            $output_ship_to_header =
                '<div class="ship_to heading">' . lang('Ship to') . ' <span class="software_highlight">'. $ship_to_name . '</span></div>';
        }
            ...
            $output_shipping_methods =
                '<table class="shipping_methods" width="100%" cellspacing="2" cellpadding="2" border="0" style="margin-bottom: 15px">
                    <tr class="heading" style="border: none">
                        <th style="text-align:left;">' . lang('Select One') . '</th>
                        <th style="text-align:right;padding-right:15px;">' . lang('Cost') . '</th>
                        <th style="text-align:left;">' . lang('Details') . '</th>
                    </tr>
                    ' . $output_shipping_method_rows . '
                </table>';
        ...
        $output =
            ...
            '<form action="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/shipping_method.php" method="post">
                ...
                <table class="shipping_info" style="width: 100%; margin-bottom: 15px">
                    <tr>
                        <td style="vertical-align: top">
                            <div class="address heading">' . lang('Shipping Address') . $for_ship_to_name . ' <a class="software_button_tiny_secondary" href="' . OUTPUT_PATH . h(encode_url_path(get_page_name($_SESSION['ecommerce']['shipping_address_and_arrival_page_id']))) . '?ship_to_id=' . h(urlencode($_GET['ship_to_id'])) . '">' . lang('Change') . '</a></div>
                            <div class="data">
                                ' . $full_name . '<br />
                                ' . $address . '
                            </div>
                ...
                <table class="products" width="100%" cellspacing="2" cellpadding="2" border="0" style="margin-bottom: 15px">
                    <tr class="products heading mobile_hide" style="border: none">
                        <th class="heading_item" style="text-align: left">' . lang('Item') . '</th>
                        <th class="heading_description" style="text-align: left">' . lang('Description') . '</th>
                        <th class="heading_selection" style="text-align: left">' . lang('Qty') . '</th>
                        <th class="heading_price" style="text-align: right">' . lang('Price') . '</th>
                        <th class="heading_amount" style="text-align: right">' . lang('Amount') . '</th>
                        <th>&nbsp;</th>
                    </tr>
                    ' . $output_products . '
                </table>
                <div class="methods heading">' . lang('Possible Shipping Methods') . $for_ship_to_name . '</div>
                ' . $output_shipping_methods . '
