...
if ($product_group['id'] == '') {
    output_error(lang('Sorry, that product group could not be found.'));
}
if($product_group['id'] != $_GET['id']){
    output_error(lang('Sorry, something went wrong, try again.'));
}
...
if (!$_POST) {
    echo
    pg_page_shell([
        'title'=> lang('Duplicate Product Group'),
                                    ...
                                    <div class="row">
                                        <div class="col-12 my-3">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type' => 'checkbox', 'id' => 'duplicate_product_groups', 'name' => 'duplicate_product_groups', 'value' => '1', 'class' => 'form-check-input')) . '
                                                <label class="form-check-label" for="duplicate_product_groups">' . lang('Also duplicate Product Groups inside this Product Group') . '.</label>
                                            </div>
                                        </div>
                                        <div class="col-12 my-3">
                                            <div class="form-check form-switch">
                                                ' . $liveform->output_field(array('type' => 'checkbox', 'id' => 'duplicate_products', 'name' => 'duplicate_products', 'value' => '1', 'class' => 'form-check-input')) . '

                                                <label class="form-check-label" for="duplicate_products">' . lang('Also duplicate Products inside this Product Group') . '.</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
// Otherwise the form has been submitted, so process it.
    ...
    log_activity(lang(array('string'=>'product group ({var:1}) was duplicated','vars'=>array($product_group['name']) )), $_SESSION['sessionusername']);

    $liveform_edit_product_group = new liveform('edit_product_group');
    $liveform_edit_product_group->add_notice(lang('The product group has been duplicated, and you are now editing the duplicate.'));

    $liveform_edit_product_group->remove_form();
