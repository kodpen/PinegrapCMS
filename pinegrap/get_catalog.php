...
function get_catalog($properties) {
    ...
    if ($layout_type == 'system') {
            ...
            $output_menu_form = '
                <div class="catalog_browse" style="float: left">
                    ...
                    <select name="product_group_id" class="software_select"><option value="' . $product_group_id . '">' . h($parent_product_group_name) . '</option>' .
                        get_product_group_options($current_product_group_id, $product_group_id, 0, 1, $product_groups = array(), $include_select_product_groups = FALSE, 'text', false, false) . '</select> <button type="submit" name="submit" value="Browse" class="software_input_submit_small_secondary" >' . lang('Browse') . '</button>
                    </form>
                </div>';
        }
        ...
        if ($search == '1') {
            ...
            if ($search_query != '') {
                $output_clear_button = '<input type="submit" name="' . $page_id . '_simple_clear" value="" class="clear" title="' . lang('Clear') . '">';
            }
        ...
        if (($mode == 'browse') && (($number_of_featured_items > 0) || ($number_of_new_items > 0))) {
            ...
            if ((count($featured_items) > 0) || (count($new_items) > 0)) {
                ...
                if ($top_item['keywords'] != '') {
                    ...
                    if ($output_keyword_links != '') {
                        $output_top_item_keywords = '<div class="keywords">' . lang('Keywords') . ': ' . $output_keyword_links . '</div>';
                    }
                }
                ...
                if (($top_item['type'] == 'product group') && ($top_item['display_type'] == 'browse')) {
                    $output_top_item_more_detail = '<div class="more_detail"><a href="' . OUTPUT_PATH . h(encode_url_path($current_page_name)) . '/' . h(encode_url_path(get_catalog_item_address_name_from_id($top_item['id'], $top_item['type']))) . '?previous_url_id=' . urlencode(generate_url_id()) . '">' . lang('More Detail') . '</a></div>';
                ...
                // so if there is a catalog detail page selected, then prepare link to catalog detail page
                    ...
                    $output_top_item_more_detail = '<div class="more_detail"><a href="' . OUTPUT_PATH . h(encode_url_path($catalog_detail_page_name)) . '/' . h(encode_url_path(get_catalog_item_address_name_from_id($top_item['id'], $top_item['type']))) . '?previous_url_id=' . urlencode(generate_url_id()) . '">' . lang('More Detail') . '</a></div>';
                }
                ...
                if ($editable) {
                    ...
                    $output_edit_container_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_edit_button_link . '?id=' . $top_item['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $tool_tip_title . ': ' . h($output_tooltip_name) . '">'.$edit_label.'</a>';
                ...
                if ((count($featured_items) + count($new_items)) > 1) {
                    ...
                    if (count($featured_items) > 1) {
                        $output_featured_items .= '<div class="heading mobile_align_left" style="border: none"><h4>' . lang('Featured') . '</h4></div>';
                        ...
                        foreach ($featured_items as $key => $featured_item) {
                            ...
                            if ($key != 0) {
                                ...
                                if ($editable) {
                                    ...
                                    // prepare edit container
                                    $output_edit_container_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_edit_button_link . '?id=' . $featured_item['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $tool_tip_title . ': ' . h($output_tooltip_name) . '">'.$edit_label.'</a>';
                                    $output_edit_container_end = '</div>';
                                }
                    ...
                    if ((count($new_items) > 1) || (($top_item_type == 'featured') && (count($new_items) > 0))) {
                        ...
                        if ($top_item_type == 'new') {
                            $output_new_items .= '<div class="heading mobile_align_left" style="border: none"><h4>' . lang('New') . '</h4></div>';
                        ...
                        foreach ($new_items as $key => $new_item) {
                            ...
                            if (($key != 0) || ($top_item_type != 'new')) {
                                ...
                                if ($editable) {
                                    ...
                                    // prepare edit container
                                    $output_edit_container_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_edit_button_link . '?id=' . $new_item['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $tool_tip_title . ': ' . h($output_tooltip_name) . '">'.$edit_label.'</a>';
                                    $output_edit_container_end = '</div>';
                                }
        ...
        if ((count($items) > 0) && ($number_of_columns > 0)) {
            ...
            foreach ($items as $key => $item) {
                ...
                if ($editable) {
                    ...
                    // prepare edit container
                    $output_edit_container_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_edit_button_link . '?id=' . $item['id'] . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $tool_tip_title . ': ' . h($output_tooltip_name) . '">'.$edit_label.'</a>';
                    $output_edit_container_end = '</div>';
                }
        ...
        // else there is not at least one item, so output message
            ...
            if ($mode == 'browse') {
                $output_items = '<div style="font-weight: bold">' . lang('There are no items in this group.') . '</div>';
                
            // else mode is search, so prepare search message
            } else {
                $output_items = '<div style="font-weight: bold">' . lang(array('string'=>'No items were found for {var:1}.','vars'=>h($search_query))) . '</div>';
            }
        }
        ...
        if ($editable == TRUE) {
            ...
            // prepare edit container
            $output_edit_container_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_product_group.php?id=' . $editable_product_group_id . '&send_to=' . h(urlencode(get_request_uri())) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . lang('Product Group') . ': ' . h($editable_product_group_name) . '">'.$edit_label.'</a><div style="padding: 2em .25em .25em .25em">';
    ...
    // Otherwise the layout is custom.
        ...
        if ($editable) {
            ...
            // prepare edit container
            $edit_start = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/edit_product_group.php?id=' . h($editable_product_group_id) . '&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . lang('Product Group') . ': ' . h($editable_product_group_name) . '">'.$edit_label.'</a><div style="padding: 2em .25em .25em .25em">';
        ...
        if ($items) {
            ...
            foreach ($items as $key => $item) {
                ...
                if ($editable) {
                    ...
                    // prepare edit container
                    $item['edit_start'] = '<div class="edit_mode" style="position: relative; outline: 1px dashed #689f38; margin: -1px;"><a href="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/' . $output_edit_button_link . '?id=' . $item['id'] . '&send_to=' . h(urlencode(REQUEST_URL)) . '" class="software_pinegrap_inline_edit_button ecommerce" title="' . $tool_tip_title . ': ' . h($output_tooltip_name) . '">'.$edit_label.'</a>';
