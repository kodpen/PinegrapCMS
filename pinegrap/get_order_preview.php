...
function get_order_preview($properties) {
    ...
    if ($layout_type == 'system') {
        ...
        // assume that the pay with iyzico payment method will not be shown until we find out otherwise
        $show_pay_with_iyzico_payment_method = FALSE;
        ...
        if (
            ...
            // if Pay with Iyzico is on and a recurring transaction is not required, then remember that the payment method should be shown
            if ((ECOMMERCE_PAY_WITH_IYZICO == TRUE) && ($recurring_transaction == FALSE)) {
                $show_pay_with_iyzico_payment_method = TRUE;
            }
            ...
            if ($show_credit_debit_card_payment_method == TRUE) {
                ...
                if (
                    ...
                    if (
                        ...
                        || ($show_pay_with_iyzico_payment_method)
                        ||
                        (
            ...
            if ($show_paypal_express_checkout_payment_method == TRUE) {
                ...
                $output_payment_methods .= '<div class="payment data" style="margin-bottom: 1em">' . $form->output_field(array('type'=>'radio', 'name'=>'payment_method', 'id'=>'payment_method_paypal_express_checkout', 'value'=>'PayPal Express Checkout', 'checked'=>$paypal_express_checkout_checked, 'class'=>'software_input_radio', 'required' => 'true')) . '<label for="payment_method_paypal_express_checkout"> <img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/paypal.png" width="40" height="26" border="0" alt="PayPal" onclick="document.getElementById(\'payment_method_paypal_express_checkout\').checked = true;"></label></div>';
            }

            // if the pay with iyzico payment method should be shown, then prepare to output it
            if ($show_pay_with_iyzico_payment_method == TRUE) {
                // if the Pay with Iyzico payment method is the only payment method, then select radio button by default
                if (
                    ($show_credit_debit_card_payment_method == FALSE)
                    && ($show_paypal_express_checkout_payment_method == FALSE)
                    &&
                    ...
                    $pay_with_iyzico_checked = 'checked';

                } else {
                    $pay_with_iyzico_checked = '';
                }

                $output_payment_methods .= '<div class="payment data" style="margin-bottom: 1em">' . $form->output_field(array('type'=>'radio', 'name'=>'payment_method', 'id'=>'payment_method_pay_with_iyzico', 'value'=>'Pay With Iyzico', 'checked'=>$pay_with_iyzico_checked, 'class'=>'software_input_radio', 'required' => 'true')) . '<label for="payment_method_pay_with_iyzico"> <img src="' . OUTPUT_PATH . OUTPUT_SOFTWARE_DIRECTORY . '/assets/images/iyzico.png" width="80" height="26" border="0" alt="' . h(lang('Pay with Iyzico')) . '" onclick="document.getElementById(\'payment_method_pay_with_iyzico\').checked = true;"></label></div>';
            }
            ...
            if (
                ...
                if (($show_credit_debit_card_payment_method == FALSE) && ($show_paypal_express_checkout_payment_method == FALSE) && ($show_pay_with_iyzico_payment_method == FALSE)) {
        ...
        // else the total is 0 or there is an active payment method, so prepare to output purchase now button
            ...
            // If Pay with Iyzico method is shown, then store label in purchase now button.
            if ($show_pay_with_iyzico_payment_method) {
                $iyzico_label = ' data-iyzico-label="' . h(lang('Continue to Iyzico')) . '"';
            }
            
            $output_purchase_now_button =
                '<div class="mobile_margin_top" style="text-align: right">
                    <input type="submit" name="submit_purchase_now" id="submit_purchase_now" value="' .  h($standard_submit_button_label) . '" class="software_input_submit_primary purchase_button"' . $paypal_label . $iyzico_label . '>
                </div>';
        }
    ...
    // Otherwise this is a custom layout.
        ...
        $pay_with_iyzico = false;
        $pay_with_iyzico_image_url = '';
        ...
        if ($recipients) {
            ...
            if (
                ...
                // If Pay with Iyzico is on and a recurring transaction is not required,
                // then remember that the payment method should be shown
                if (ECOMMERCE_PAY_WITH_IYZICO and !$recurring_transaction) {
                    $pay_with_iyzico = true;
                    $number_of_payment_methods++;
                }
                ...
                if ((ECOMMERCE_CREDIT_DEBIT_CARD == true) && ((ECOMMERCE_AMERICAN_EXPRESS === true) || (ECOMMERCE_DINERS_CLUB == true) || (ECOMMERCE_DISCOVER_CARD == true) || (ECOMMERCE_MASTERCARD == true) || (ECOMMERCE_VISA == true))) {
                    ...
                    // default.
                    if (!$paypal_express_checkout and !$pay_with_iyzico and !$offline_payment) {
                    ...
                    if (
                        ...
                        // because the surcharge might not have been included in the totals
                        // above until the customer selected the credit card payment method.
                        if ($paypal_express_checkout or $pay_with_iyzico or $offline_payment) {
                ...
                if ($paypal_express_checkout) {
                    ...
                    $paypal_express_checkout_image_url = PATH . SOFTWARE_DIRECTORY . '/assets/images/paypal.png';
                }

                // If Pay with Iyzico payment method should be shown, then prepare.
                if ($pay_with_iyzico) {

                    // If Pay with Iyzico is the only payment method,
                    // then select it by default.
                    if (
                        !$credit_debit_card
                        ...
                        $form->set('payment_method', 'Pay With Iyzico');
                    }

                    $pay_with_iyzico_image_url = PATH . SOFTWARE_DIRECTORY . '/assets/images/iyzico.png';
                }
        ...
        $content = render_layout(array(
            ...
            'pay_with_iyzico' => $pay_with_iyzico,
            'pay_with_iyzico_image_url' => $pay_with_iyzico_image_url,
