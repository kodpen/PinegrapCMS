...
function get_calendar_event_view_screen_content($properties)
{
    ...
    if ((isset($_GET['from']) == false) || (($_GET['from'] ?? '') != 'control_panel')) {
        ...
        if ($calendar_event == FALSE) {
            output_error(lang('The requested calendar event could not be found. <a href="javascript:history.go(-1)">Go back</a>.'));
        }
        
        // if calendar event is not published, output error
        if ($calendar_event['published'] == 0) {
            output_error(lang('The requested calendar event is not currently published. <a href="javascript:history.go(-1)">Go back</a>.'));
        }
        ...
        if (mysqli_num_rows($result) == 0) {
            output_error(lang('The requested calendar event is not allowed in this calendar event view. <a href="javascript:history.go(-1)">Go back</a>.'));
        }
