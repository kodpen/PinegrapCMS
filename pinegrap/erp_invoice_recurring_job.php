 ...
 *
 * Scheduled job: repeating invoices. Writes every invoice a recurrence has
 * come due for, as a draft or issued, the way the recurrence says. The work
 * lives in includes/erp/invoice_recurring.php; this script only decides who
 * may start it.
 *
...
if (defined('ERP_ENABLED') && ERP_ENABLED) {
    ...
    $result = erp_invoice_recurring_run();
}

if (function_exists('pg_cron_ran')) {
    pg_cron_ran('erp_invoice_recurring_job');
}

if (php_sapi_name() === 'cli') {
    print 'repeating invoices: ' . (int) $result['written'] . ' written, ' . (int) $result['skipped'] . ' skipped'
        . (!empty($result['errors']) ? (', ' . count($result['errors']) . ' error(s): ' . implode('; ', $result['errors'])) : '') . "\n";
} elseif (!pg_cron_is_background_run()) {
    go(PATH . SOFTWARE_DIRECTORY . '/erp_invoice_recurrences.php');
}
