    ...
    if ($_POST['quick_add_product_id']) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            // else the product is not available, so add error
                ...
                $liveform->mark_error('quick_add_product_id', lang(array('string' => 'Sorry, {var:1} is not currently available.', 'vars' => h($product_description))));
            }
...
// if purchase now button was clicked, or Continue to PayPal was clicked, or
// this is a return from PayPal / 3DS / Pay With Iyzico, then the order has
// been submitted and we must continue to submit_order.php (otherwise the
// no-submit branch at the bottom redirects to home, because $_POST['page_id']
        ...
        for ($quantity_number = 1; $quantity_number <= $number_of_forms; $quantity_number++) {
            ...
            foreach ($form_fields as $form_field) {
                ...
                if (
                    ...
                    if (!$reference_code) {
                        $liveform->mark_error($html_field_name, lang(array('string' => 'Sorry, the value you entered for {var:1} is not valid.', 'vars' => $form_field['label'])));
                    }
                }
...
if ($order_submitted == false) {
    // page_id falls back to GET because gateway callbacks (Iyzico /
    // PayPal / 3DS) come in via a fresh HTTP request that puts the id
    // on the query string, not in $_POST. Without the GET fallback this
    // redirect would land on the site homepage (empty page name → PATH
    // alone) instead of the express order page.
    $_eo_redirect_pid = !empty($_POST['page_id']) ? (int)$_POST['page_id']
                       : (!empty($_GET['page_id'])  ? (int)$_GET['page_id']  : 0);
    header('Location: ' . URL_SCHEME . $_SERVER['HTTP_HOST'] . PATH . get_page_name($_eo_redirect_pid));
    exit();
