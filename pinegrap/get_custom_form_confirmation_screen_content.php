...
function get_custom_form_confirmation_screen_content($properties)
{
    ...
    // so deal with that.
        ...
        if ($_GET['r'] == '') {
            output_error(lang('Sorry, it appears that a reference code was not included in the website address, so we don\'t know which confirmation to show you.'));
        }
    ...
    if (mysqli_num_rows($result) == 0) {
        output_error(lang('Sorry, we can\'t show you the confirmation because we can\'t find the form. The form might have recently been deleted.'));
    }
