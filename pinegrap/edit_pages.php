...
if (!$_POST) {
    ...
    $default_style_label = lang('Default') . ' (' . lang('inherit') . ')';
    ...
    if (
        ...
        <div class="col-12">
            <div class="alert alert-success" role="alert">
                <h4 class="alert-heading">' . lang('Theme Preview Mode') . '</h4>
                <p>' . lang('You are currently previewing a Theme that is not activated, so your Page Style and Mobile Page Style selections below will update the preview Page Style, and not the activated Page Style.  The preview Page Styles that you set here will be activated when you activate the Theme.') . '</p>
            </div>
        </div>';

        $default_style_label = '[' . lang('Activated') . ']';
    }
    ...
    print
    output_header_secure(array('title'=>lang('Modify Pages'),'icon'=>'page')) . '
    <nav id="header" class="navbar sticky-top rounded-0 navbar-expand border-bottom shadow-sm bg-body">
        <div class="container-fluid">
            <span class="navbar-text me-auto" data-bs-content="' . lang('You may update various properties below for the selected pages. You may leave a pick list unselected if you do not want to modify a property.') . '" title="' . lang('Modify Pages') . '">' . lang('Modify Pages') . '</span>
        ...
        <div class="row">
            ...
            <div class="col-12">
                <div class="row">
                    ...
                    <div class="col-12 col-sm-6">
                        <div class="form-floating mt-1 mb-2">
                            <select class="form-select" id="page_style"><option value=""></option>' . select_style('', $default_style_label) . '</select>
                            <label for="page_style" class="form-label">' . lang('Page Style') . '</label>
                        </div>
                    </div>
                    <div class="col-12 col-sm-6">
                        <div class="form-floating mt-1 mb-2">
                            <select class="form-select" id="mobile_style_id"><option value=""></option>' . get_mobile_style_options('', $default_style_label) . '</select></select>
            ...
            <div class="col-12 col-sm-6">
                <div class="form-floating mt-1 mb-2">
                    ' . $liveform->output_field(array('type'=>'select', 'id'=>'site_search','class'=>'form-select', 'options'=>$site_search_options)) . '
                    <label for="folder" class="form-label">' . lang('Site Search') . '</label>
                </div>
            </div>
            <div class="col-12 col-sm-6">
                <div class="form-floating mt-1 mb-2">
                    ' . $liveform->output_field(array('type'=>'select', 'id'=>'sitemap','class'=>'form-select', 'options'=>$sitemap_options)) . '
                    <label for="folder" class="form-label">sitemap.xml</label>
                </div>
            </div>
            <nav class="buttons navigation text-center position-sticky" style="bottom:.5rem;" aria-label="data edit buttons ">
                <div class="container">
                    <div class=" btn-group btn-group-sm flex-wrap justify-content-center mb-0">
                        <button type="button" name="submit_save" value="Modify Pages" class="btn mb-1 mt-1  btn-primary submit-primary" onclick="edit_pages()"><span class="material-icons me-2">save</span>' . lang(array('string'=>'Modify Pages') ) . '</button>
                    </div>
                </div>
            </nav>
        ...
        switch ($_POST['action'] ?? '') {
            ...
            case 'edit':
                ...
                if (($_POST['move_to_folder']) && (check_edit_access($_POST['move_to_folder']) == false)) {
                    output_error(lang('You do not have access to move pages to the folder that you selected') . '. <a href="javascript:history.go(-1);">' . lang('Go back') . '</a>.');
                }
                ...
                foreach ($_POST['pages'] as $page_id) {
                    ...
                    // else output error
                        ...
                        output_error(lang('You do not have access to edit the pages that you selected') . '. <a href="javascript:history.go(-1);">' . lang('Go back') . '</a>.');
                    }
                }
                ...
                if ($number_of_pages > 0) {
                    ...
                    if ($_POST['edit_page_style'] != '') {
                        ...
                        if ($_POST['edit_page_style'] == '0') {
                            ...
                            if (
                                ...
                                $style = '[' . lang('Activated') . ']';

                            } else {
                                $style = lang('Default') . ' (' . lang('inherit') . ')';
                            }
                    ...
                    if ($_POST['edit_mobile_style_id'] != '') {
                        ...
                        if ($_POST['edit_mobile_style_id'] == '0') {
                            ...
                            if (
                                ...
                                $mobile_style = '[' . lang('Activated') . ']';

                            } else {
                                $mobile_style = lang('Default') . ' (' . lang('inherit') . ')';
                            }
                        ...
                        // output message for log
                        $log_message .= lang(array('string'=>'had mobile {var:1}page style changed to {var:2}','vars'=>array($preview,$mobile_style) ));
                    ...
                    if ($_POST['edit_site_search'] != '') {
                        ...
                        // output message for log
                        $log_message .= lang(array('string'=>'had site search turned {var:1}','vars'=>array($on_off) ));
                    ...
                    if ($_POST['edit_sitemap'] != '') {
                        ...
                        if ($_POST['edit_sitemap'] == '1') {
                            $added_to_or_removed_from = lang('added to');
                        } else {
                            $added_to_or_removed_from = lang('removed from');
                        }
                        
                        // output message for log
                        $log_message .= lang(array('string'=>'were {var:1} sitemap.xml','vars'=>array($added_to_or_removed_from) ));
                    }
                    
                    // if there is a log message, then log it and add notice
                    if ($log_message != '') {
                        log_activity(lang(array('string'=>'{var:1} page(s) {var:2}','vars'=>array($number_of_pages,$log_message) )), $_SESSION['sessionusername']);
                        $liveform->add_notice(lang(array('string'=>'{var:1} page(s) {var:2}','vars'=>array($number_of_pages,$log_message) )) );
                    }
                }
            ...
            case 'delete':
                ...
                if ($number_of_pages > 0) {
                    log_activity(lang(array('string'=>'{var:1} page(s) were deleted','vars'=>$number_of_pages)), $_SESSION['sessionusername']);
                    $liveform->add_notice( lang(array('string'=>'{var:1} page(s) were deleted','vars'=>$number_of_pages)) );
                }
                
                if ($access_error_exists == true) {
                    $liveform->mark_error('access_error', lang('At least one page could not be deleted, because you do not have access to the page(s).'));
                }
                
                if ($submitted_forms_error_exists == true) {
                    $liveform->mark_error('submitted_forms_error', lang('At least one page could not be deleted, because there are submitted forms for the page(s).  All submitted forms for the page(s) must be deleted before the page(s) are allowed to be deleted.'));
                }
