...
// the same rule every other upload door applies.
if (pg_upload_name_blocked($_FILES['file']['name'])) {
    output_error(h(pg_upload_blocked_message($_FILES['file']['name'])) . ' <a href="javascript:history.go(-1)">Go back</a>.');
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($_POST['overwrite'] == 'true') {
        ...
        // else user does not have access to overwrite file, so output error
        } else {
            output_error('You do not have access to overwrite the file. <a href="javascript:history.go(-1)">Go back</a>.');
        }
...
header('Location: ' . URL_SCHEME . $_SERVER['HTTP_HOST'] . PATH . SOFTWARE_DIRECTORY . '/tiny_mce_3_5_10/plugins/advlink/link.htm?page_id=' . $_POST['page_id'] . '&file_id=' . $file_id . '&file_url=' . PATH . $file_name);
?>