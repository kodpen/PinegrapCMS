...
(function () {
    ...
    // Profile edit link for a user (name/surname → linked contact, account
    // → user edit). Server-computed permission flags decide whether a link
    // is rendered at all — nobody gets a link that ends in "Access denied";
    // the edit pages still re-check on their own. Returns '' when the
    // viewer cannot edit either record.
    function profileHref(user) {
        var urls = config.urls || {};

        if (user.can_edit_contact && user.contact_id && urls.edit_contact) {
            return urls.edit_contact + user.contact_id;
        }

        if (user.can_edit_user && urls.edit_user) {
            return urls.edit_user + user.id;
        }
    ...
    // Opens in a new tab so the chat panel stays where it is; the click
    // must not bubble into the row (which would start a conversation).
    function profileLink(className, text, href) {
        var link = el('a', className + ' pg-chat-link', text);
    ...
    function userRow(user, onClick) {
        ...
        // The name links to the profile when the viewer may edit it (the
        // row click still opens the conversation).
        var nameHref = profileHref(user);
        var name = nameHref
            ? profileLink('text-truncate me-1 fw-semibold', user.name, nameHref)
            : el('span', 'text-truncate me-1 fw-semibold', user.name);
        ...
        // "Last seen" rides on the presence line; d-block + truncate keeps
        // long labels from wrapping the row (full text in the tooltip).
        var presence = el('small', 'text-muted d-block text-truncate', presenceText(user));
    ...
    function openConversationView(conversation) {
        ...
        var headerNameHref = headerPeer ? profileHref(headerPeer) : '';

        if (headerNameHref) {
            titleLine.appendChild(profileLink('', state.conversation.title, headerNameHref));
        } else {
            titleLine.textContent = state.conversation.title;
        }

        if (headerPeer && headerPeer.username && accountHref(headerPeer)) {
            var accountLink = profileLink('text-muted ms-1', '@' + headerPeer.username, accountHref(headerPeer));
            accountLink.style.fontSize = '11px';
            accountLink.style.fontWeight = 'normal';
            titleLine.appendChild(accountLink);
        }
