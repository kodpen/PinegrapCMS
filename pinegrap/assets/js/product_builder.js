...
(function ($) {
    ...
    /* A 2x1 JPEG tagged Orientation=6. Used once, by orientationApplied(), to
       find out whether this browser turns that tag into pixels. */
    var ORIENTATION_PROBE = '';

    var $form = $('#pg_pb_form');

    /* The image picker is drawn on the variant set screens too, and those have
       no #pg_pb_form of their own — pg_pb_render_image_picker() emits the same
       markup there and the same code has to drive it, otherwise the "Upload"
       button and the drop area are decoration.

       So the guard asks whether there is anything here for this file at all,
       and the half that belongs to the product form returns further down, once
       the picker is wired. */
    var $imagePicker = $('#pg_pb_image_drop');

    if (!$form.length && !$imagePicker.length) {
        return;
    }
    ...
    /* Bytes as the operator reads them. The server has convert_bytes_to_string()
       for the same job; this is the one place the browser needs it, and passing
       formatted strings back through the JSON would mean the numbers could not
       be added up here. */
    ...
    /* What the upload did, in the box under the drop area rather than in a
       toast.

       A toast is the wrong shape for this. The saving is worth seeing but not
       worth acknowledging, and the "this one is too small" line has to stay on
       screen while the operator decides what to do about it — a message that
       fades after four seconds is one the operator scrolls back for and cannot
       find. */
    function reportUpload(result) {

        var $notice = $('#pg_pb_image_notice');
        ...
        $.each(result.files || [], function (index, file) {
            ...
            count++;
            before += Number(file.original_size) || 0;
            after  += Number(file.size) || 0;

            if (file.too_small) {
                small.push(file.name);
            }
        });

        /* Failures first. A batch where four photos landed and one did not is
           reported as both, in that order, because the one that did not is the
           part the operator has to do something about. */
        if ((result.tooBig || []).length) {
            output +=
                '<div class="alert alert-danger py-2 mb-2">' +
                esc(label('upload_too_big')
                    .replace('{limit}', readableBytes(result.fileMax || 0))
                    .replace('{files}', result.tooBig.join(', '))) +
                '</div>';
        }

        $.each(result.failed || [], function (index, failure) {
            output +=
                '<div class="alert alert-danger py-2 mb-2">' + esc(failure.message || '') + '</div>';
        });

        if (small.length) {
            output +=
                '<div class="alert alert-warning py-2 mb-2">' +
                esc(label('image_too_small')
                    .replace('{min}', result.minimum_dimension || 0)
                    .replace('{files}', small.join(', '))) +
                '</div>';
        }
        ...
        if (result.shrunk) {
            output +=
                ...
                esc(label('images_shrunk_in_browser').replace('{count}', result.shrunk)) +
                '</div>';
        }

        /* Only when there was a saving worth a sentence. A file that was
           already small gives up a few kilobytes, and announcing that as a
           result trains the operator to ignore the box that also carries the
           warnings above it. */
        if (count && ((before - after) > (50 * 1024))) {
            output +=
                '<div class="alert alert-success py-2 mb-0">' +
                esc(label('images_optimized')
                    .replace('{count}', count)
                    .replace('{before}', readableBytes(before))
                    .replace('{after}', readableBytes(after))) +
                '</div>';
        }
    ...
    /* A file that fits goes up untouched. This is only for the one that would
       otherwise be refused.

       The server resamples with Lanczos and re-encodes from the original
       bytes, which is better than anything a canvas will do — so a photo under
       the server's ceiling is never touched here. But a 10 MB photo on a host
       with an 8 MB post_max_size cannot reach that code at all, and a slightly
       worse copy that arrives beats a better one that does not. */

    var SHRINKABLE_TYPES = { 'image/jpeg': 1, 'image/png': 1, 'image/webp': 1 };

    function shrinkableType(file) {

        if (SHRINKABLE_TYPES[file.type]) {
            return file.type;
        }

        /* Some browsers hand over an empty type for a file dragged out of an
           archive or a network share. */
    ...
    /* Whether this browser turns an EXIF rotation into pixels when asked.

       It matters more than it looks. A canvas has no metadata, so whatever
       comes off it is final: if the browser decoded a sideways phone photo
       without applying its Orientation tag, the upload is a sideways picture
       with nothing left to say so, and the server's own correction has no tag
       to read. There is no way to feature-detect an ImageBitmapOptions member,
       so this asks the browser directly — a 2x1 JPEG tagged "rotate 90" comes
       back 1x2 from a browser that honours it.

       Answered once, and a browser that fails is simply not asked to shrink
       JPEGs; PNG and WebP carry no orientation and are unaffected. */
    var orientationProbe = null;

    function orientationApplied() {

        if (orientationProbe) {
            return orientationProbe;
        }

        orientationProbe = new Promise(function (resolve) {

            try {
                var raw   = window.atob(ORIENTATION_PROBE);
                ...
                window.createImageBitmap(new Blob([bytes], { type: 'image/jpeg' }),
                    { imageOrientation: 'from-image' })
                    .then(function (bitmap) {
                        var applied = (bitmap.height > bitmap.width);
    ...
    function drawScaled(bitmap, edge) {
        ...
        /* A white ground for JPEG: the format has no alpha, and a transparent
           PNG drawn straight onto an empty canvas and encoded as JPEG comes
           out on black. */
    ...
    /* Encode, and if it still does not fit, try again with less. Quality first,
       then pixels — a photo at 2400 px and quality 0.6 reads better than the
       same photo at 1400 px and quality 0.85. PNG has no quality knob, so for
       it only the pixels can move. */
    function encodeWithinLimit(bitmap, type, edge, quality, limit) {

        var canvas = drawScaled(bitmap, edge);
        ...
        return canvasToBlob(canvas, type, quality).then(function (blob) {
            ...
            if (type === 'image/png') {
                return (edge > 800)
                    ? encodeWithinLimit(bitmap, type, Math.round(edge * 0.75), quality, limit)
                    : null;
            }

            if (quality > 0.55) {
                return encodeWithinLimit(bitmap, type, edge, Math.max(0.55, quality - 0.12), limit);
            }

            return (edge > 800)
                ? encodeWithinLimit(bitmap, type, Math.round(edge * 0.75), 0.72, limit)
                : null;
        });
    }

    /* Resolves with an upload item, or with null when this file cannot be made
       to fit — in which case it is reported by name rather than sent. */
    function shrinkFile(file, edge, limit) {

        var type = shrinkableType(file);

        if (!edge || !type || !canShrink()) {
            return Promise.resolve(null);
        }

        var guard = (type === 'image/jpeg') ? orientationApplied() : Promise.resolve(true);
        ...
        return guard.then(function (ok) {
            ...
            return window.createImageBitmap(file, { imageOrientation: 'from-image' })
                .then(function (bitmap) {
                    ...
                    var quality = (Number(uploadLimits.shrinkQuality) || 82) / 100;

                    return encodeWithinLimit(bitmap, type, edge, quality, limit)
                        .then(function (blob) {
    ...
    /* Everything the batching code works on, in one shape: what to send, under
       what name, and how big it is. */
    function prepareFiles(files, done) {

        var fileMax = Number(uploadLimits.fileMax) || 0;
        var edge    = Number(uploadLimits.shrinkTo) || 0;
        var ready   = [];
        var tooBig  = [];
        var shrunk  = 0;
        var index   = 0;
        ...
        function step() {
            ...
            if (index >= files.length) {
                done({ ready: ready, tooBig: tooBig, shrunk: shrunk });
                return;
            }
            ...
            if (!fileMax || (size <= fileMax)) {
                ready.push({ blob: file, name: file.name, size: size, shrunk: false });
                step();
                return;
            }

            shrinkFile(file, edge, fileMax).then(function (item) {
    ...
    /* What this server will take in one request, handed over by PHP.
       Zeros mean "no limit known", which is how this behaves on a screen that
       does not send the block. */
    var uploadLimits = cfg.uploadLimits || {};

    /* Split a selection into requests the server will actually accept.

       Everything used to go up in one request. PHP compares that request
       against post_max_size and, when it loses, throws the whole body away
       before add_file.php runs — so the answer came back as an HTML error
       page, arrived here as a parse failure, and the operator was told
       "Sorry, we could not accept your request" about eight photos that were
       individually fine.

       Splitting is the fix rather than a bigger limit, because the limit
       belongs to the server and most operators cannot change it. */
    function uploadBatches(files) {

        var requestMax = Number(uploadLimits.requestMax) || 0;
        var maxFiles   = Number(uploadLimits.maxFiles) || 0;
        var batches    = [];
        var batch      = [];
        var total      = 0;
        var i, size;
        ...
        for (i = 0; i < files.length; i++) {
            ...
            size = Number(files[i].size) || 0;

            /* batch.length is checked first so a single file over the limit
               still gets its own request — it will be refused, but by the
               server, with a sentence about that file. */
            if (batch.length
                && (((requestMax > 0) && ((total + size) > requestMax))
                    || ((maxFiles > 0) && (batch.length >= maxFiles)))
            ) {
                batches.push(batch);
                batch = [];
                total = 0;
    ...
    /* One request. Resolves with the parsed response; never rejects, because a
       batch that fails must not take the batches after it down with it. */
    function sendBatch(files, onProgress) {

        var form = new FormData();
        var i;

        /* The third argument is the filename. It matters for a shrunk photo:
           what goes up is a Blob produced by a canvas, which has no name of
           its own, and without this the server stores it as "blob". */
        for (i = 0; i < files.length; i++) {
            form.append('file[]', files[i].blob, files[i].name);
        }
        ...
        /* What marks these as product photos rather than files. add_file.php
           compresses them and scales anything over the ceiling; without this
           field the same script stores the bytes untouched, which is what
           every other upload on the site still gets. */
        form.append('image_profile', 'product');

        return $.ajax({
            /* jsonreturn is in the query string as well as the body. When the
               body is over post_max_size the body is gone, and that is exactly
               the request that most needs a JSON answer rather than an HTML
               error page. */
            url:         'add_file.php?jsonreturn=true',
            type:        'POST',
            data:        form,
            dataType:    'json',
            ...
            if (!response || !response.files) {
                return { files: [], failed: [{ message: label('request_failed') }] };
            }
            ...
            /* add_file.php answers a request it could not take with a JSON
               body and a 413, so the reason survives. An expired token or a
               folder the operator cannot write to still arrives as an HTML
               error page, which is the fallback sentence. */
            ...
            return $.Deferred().resolve({
                files:  [],
                failed: [{ message: (payload && payload.message) ? payload.message : label('request_failed') }]
            }).promise();
        });
    }
    ...
    function uploadFiles(files) {
        ...
        /* Shown before preparation, not after. Shrinking a 10 MB photo takes a
           moment, and a screen that does nothing visible while it happens is a
           screen the operator clicks again. */
        $progress.removeClass('d-none');
        $bar.css('width', '0%');

        prepareFiles(files, function (prepared) {

            var sending = prepared.ready;
            ...
            if (!sending.length) {
                ...
                reportUpload({
                    files:  [],
                    failed: [],
                    tooBig: prepared.tooBig,
                    shrunk: prepared.shrunk,
                    fileMax: Number(uploadLimits.fileMax) || 0
                });
                return;
            }

            var batches    = uploadBatches(sending);
            var grandTotal = 0;
            var doneBytes  = 0;
            var i;

            for (i = 0; i < sending.length; i++) {
                grandTotal += sending[i].size;
            }
            ...
            /* Sequential, not parallel. The batches exist because the server
               has a ceiling on one request; firing them together would put the
               same bytes on the wire at the same moment and, on a server that
               also caps concurrent uploads, trade one failure for several. */
            function next(index) {

                if (index >= batches.length) {
                    $progress.addClass('d-none');
                    reportUpload(collected);
                    return;
                }
                ...
                for (j = 0; j < batches[index].length; j++) {
                    batchBytes += batches[index][j].size;
                }

                sendBatch(batches[index], function (loaded) {
                    if (grandTotal > 0) {
                        $bar.css('width', Math.min(100, Math.round(((doneBytes + loaded) / grandTotal) * 100)) + '%');
                    }
                    ...
                    if (response) {
                        ...
                        if (response.minimum_dimension) {
                            collected.minimum_dimension = response.minimum_dimension;
                        }

                        $.each(response.files || [], function (position, file) {

                            collected.files.push(file);

                            /* Routed through the shared picker so the tile is
                               built by the same code as every other one — cover
                               badge, sort handle and remove button included. */
                        ...
                        $.each(response.failed || [], function (position, failure) {
                            collected.failed.push(failure);
                        });
                    }
    ...
    /* Everything past this point belongs to the product screen: the variant
       matrix, the attribute cards, the live preview. The variant set screens
       draw the picker and nothing else, so they stop here. */
    if (!$form.length) {
        return;
    }
